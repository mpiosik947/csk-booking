...
test("all four screens use bounded RPC reads without browser fetch-all or N+1", async () => {
  ...
  assert.match(publicPage, /get_public_event_list_v3/);
  assert.match(adminPage, /admin_list_events_v2/);
  assert.match(adminPage, /admin_list_event_registrations_v2/);
  assert.match(myPage, /get_my_event_registrations_v2/);
