...
test("local validation runs before each Supabase password mutation", async () => {
  ...
  const recovery = await readFile(new URL("../app/auth/recovery/route.ts", import.meta.url), "utf8");
  assert.ok(reset.indexOf("getPasswordLengthError(password)") < reset.indexOf('method: "POST"'));
  assert.ok(recovery.indexOf("getPasswordLengthError(password)") >= 0);
  assert.ok(recovery.indexOf("getPasswordLengthError(password)") < recovery.indexOf("client.auth.updateUser"));
  assert.ok(
