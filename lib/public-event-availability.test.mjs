...
test("events page uses only the bounded authoritative public event RPC", async () => {
  const source = await readFile(
    new URL("../app/events/page.tsx", import.meta.url),
    "utf8"
  );

  assert.match(source, /\.rpc\(\s*"get_public_event_list_v2"/);
  assert.match(source, /p_page_size: EVENT_LIST_PAGE_SIZE/);
  assert.match(source, /parsePublicEventList\(data\)/);
  assert.doesNotMatch(source, /get_public_event_availability_v1/);
...
test("event registration write path remains atomic and unchanged", async () => {
  ...
  assert.match(source, /fetch\([\s\S]*?\/api\/register-event\?tenant=/);
  assert.match(source, /"\/api\/register-event"/);
...
test("public event CTA closes at the canonical Warsaw start without changing mutations", async () => {
  const source = await readFile(
    new URL("../app/events/page.tsx", import.meta.url),
    "utf8"
  );
  ...
  assert.match(source, /\.rpc\(\s*"get_public_event_list_v2"/);
  assert.match(source, /\.rpc\(\s*"get_public_event_list_v3"/);
  assert.match(source, /await fetchPublicEvents\(search, page, tenantId\)/);
});
