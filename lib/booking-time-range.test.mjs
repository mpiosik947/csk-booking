...
test("lane_blocked invalidates the selected start and refreshes without retrying", async () => {
  ...
  assert.equal(
    (source.match(/\/api\/create-reservation\?tenant=/g) ?? []).length,
    1
  );
  assert.doesNotMatch(conflictBranch, /handleSubmit\(|fetch\(/);
});

test("V3 availability keeps preparation errors controlled without retry or refresh", async () => {
  const source = await readFile(
    new URL("../app/booking/BookingForm.tsx", import.meta.url),
    "utf8"
  );
  const controlledBranch = source.match(
    /if \(\s*result\.code === "contact_required" \|\|\s*result\.code === "lane_not_bookable"\s*\) \{[\s\S]*?\n\s*return;\s*\}/
  )?.[0] ?? "";

  assert.notEqual(controlledBranch, "");
  assert.match(source, /\.rpc\(\s*"get_lane_booking_busy_ranges_v3"/);
  assert.doesNotMatch(source, /get_lane_booking_busy_ranges_v2/);
  ...
  assert.equal((source.match(/\/api\/create-reservation\?tenant=/g) ?? []).length, 1);
