...
test("my reservations uses only the ownership RPC for its main read", async () => {
  ...
  assert.match(source, /\.rpc\("get_my_reservations_v3",\s*\{[\s\S]*?p_tenant_id: tenantId,[\s\S]*?p_page_size: to - from \+ 1/);
...
test("tenant-scoped cancellation and check-in token flows remain in place", async () => {
  const source = await readFile(
    new URL("../app/my-reservations/page.tsx", import.meta.url),
    "utf8"
  );

  assert.match(source, /tenant-cancel-reservation\?tenant=/);
  assert.doesNotMatch(source, /\.rpc\("cancel_reservation"/);
...
test("my reservations shows the exact Warsaw deadline and hides self-cancel after cutoff", async () => {
  const source = await readFile(
    new URL("../app/my-reservations/page.tsx", import.meta.url),
    "utf8"
  );

  assert.match(source, /RESERVATION_CANCELLATION_CUTOFF_HOURS/);
  assert.match(source, /formatWarsawCancellationDeadline/);
  assert.match(source, /isBeforeWarsawCancellationCutoff/);
