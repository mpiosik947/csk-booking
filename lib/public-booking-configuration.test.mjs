...
test("booking page uses only the scoped or legacy public configuration RPC", async () => {
  const source = await readFile(
    new URL("../app/booking/page.tsx", import.meta.url),
    "utf8"
  );

  assert.match(source, /\.rpc\(\s*"get_public_booking_configuration_v2"\s*,\s*\{ p_tenant_id: tenantId \}\s*\)/);
  assert.match(source, /\.rpc\(\s*"get_public_booking_configuration_v2"\s*,\s*\{ p_tenant_id: tenantId \}/);
