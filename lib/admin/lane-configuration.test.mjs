...
function validCreateState(name = "Nowa oś") {
  const state = createInitialLaneFamilyCreateState();
  state.root.name = name;
  state.root.max_shooters = "2";
  state.root.max_people_online = "2";
  state.root.pricing = [
    editPricing("mon_thu", 1, 2, "Pon–Czw", 100, "create-root-m"),
    editPricing("fri_sun", 1, 2, "Pt–Nd", 120, "create-root-f"),
  ];
  return state;
}

function configureCreatePosition(state, index, name = `Stanowisko ${index + 1}`) {
  const position = state.positions[index];
  position.name = name;
  position.pricing = [
    editPricing("mon_thu", 1, 1, "Pon–Czw", 60, `${position.edit_key}-m`),
    editPricing("fri_sun", 1, 1, "Pt–Nd", 70, `${position.edit_key}-f`),
  ];
  return position;
}
...
test("builds a standalone create payload without client-provided UUIDs", () => {
  const state = validCreateState("  Oś Żółta  ");
  const payload = buildLaneFamilyCreatePayload(state);

  assert.equal(validateLaneFamilyCreateState(state).valid, true);
  assert.equal(payload.root.name, "Oś Żółta");
  assert.equal(payload.root.is_active, false);
  assert.equal(payload.root.online_bookable, false);
  assert.equal(payload.root.whole_lane_bookable, true);
  assert.equal(payload.root.positions_bookable, false);
  assert.deepEqual(payload.root.durations_minutes, [60]);
  assert.equal(payload.positions.length, 0);
  assert.doesNotMatch(JSON.stringify(payload), /lane_id|parent_lane_id|resource_kind/);
});

test("creates dynamic position drafts and preserves generated presentation names", () => {
  let state = validCreateState("Rodzina dynamiczna");
  state = addPositionToLaneFamilyCreateState(state);
  state = addPositionToLaneFamilyCreateState(state);
  configureCreatePosition(state, 0);
  configureCreatePosition(state, 1);
  ...
  assert.deepEqual(
    payload.positions.map((position) => position.name),
    ["Stanowisko 1", "Stanowisko 2"]
  );
});

test("create validation allows Polish Unicode and rejects unsafe names", () => {
  const valid = validCreateState("Oś Żółć — Północ");
  assert.equal(validateLaneFamilyCreateState(valid).valid, true);

  for (const invalidName of ["   ", "<b>Oś</b>", "Oś\nDruga", "x".repeat(121)]) {
    const state = validCreateState(invalidName);
    assert.equal(validateLaneFamilyCreateState(state).valid, false);
    assert.throws(() => buildLaneFamilyCreatePayload(state), /invalid_create_state/);
  }
});

test("create validation rejects pricing gaps, overlaps and missing day groups", () => {
  const cases = [
    [
      editPricing("mon_thu", 1, 1, "one", 10, "gap-1"),
      editPricing("mon_thu", 3, 3, "three", 30, "gap-2"),
      editPricing("fri_sun", 1, 2, "all", 40, "gap-3"),
    ],
    [
      editPricing("mon_thu", 1, 2, "first", 10, "overlap-1"),
      editPricing("mon_thu", 2, 2, "second", 30, "overlap-2"),
      editPricing("fri_sun", 1, 2, "all", 40, "overlap-3"),
    ],
    [editPricing("mon_thu", 1, 2, "only", 10, "missing")],
  ];
  for (const pricingRules of cases) {
    const state = validCreateState();
    state.root.pricing = pricingRules;
    assert.equal(validateLaneFamilyCreateState(state).valid, false);
  }
});

test("create validation enforces hierarchy activation and capacity invariants", () => {
  let state = validCreateState();
  state = addPositionToLaneFamilyCreateState(state);
  const position = configureCreatePosition(state, 0);
  position.is_active = true;
  assert.match(
    validateLaneFamilyCreateState(state).errors.join(" "),
    /Nieaktywna oś/
  );
  ...
  assert.match(
    validateLaneFamilyCreateState(state).errors.join(" "),
    /rezerwacji stanowisk/i
  );

  state.root_positions_bookable = true;
  position.max_shooters = "3";
  position.max_people_online = "1";
  assert.match(
    validateLaneFamilyCreateState(state).errors.join(" "),
    /Suma pojemności stanowisk/
  );
});

test("parses only the exact create result contract", () => {
  const created = parseLaneFamilyCreateResult({
    ok: true,
    changed: true,
    code: "created",
    root_lane_id: ROOT_ID,
    configuration_version: 1,
    created_resource_count: 3,
  });
