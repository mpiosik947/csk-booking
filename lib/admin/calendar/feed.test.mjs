...
const scopeModuleUrl = await compileModule("./scope.ts");
...
test("parent-family filter keeps a single-position reservation exactly once", () => {
  const result = feed(
    {
      lanes: [
        { ...lane(PARENT), name: "Oś rodzic", positions_bookable: true },
        position(CHILD),
        position(testLaneId("2"), PARENT, "Stanowisko 2", true, 20),
      ],
      reservations: [reservation("r-child", CHILD, "10:00", "11:00")],
    },
    { laneId: PARENT },
  ...
  assert.equal(reservations[0].laneId, CHILD);
  assert.equal(reservations[0].laneName, "Oś rodzic — Stanowisko 1");
  assert.equal(result.lanes.some((resource) => resource.id === CHILD), true);
  assert.equal(
    reservations.some((entry) => entry.laneId === testLaneId("2")),
    false,
  );
...
test("parent-family filter keeps a historical reservation on an inactive child", () => {
  const result = feed(
    {
      lanes: [
        { ...lane(PARENT), name: "Oś rodzic", positions_bookable: true },
        position(INACTIVE_CHILD, PARENT, "Stanowisko historyczne", false, 20),
      ],
      reservations: [
        reservation("r-history", INACTIVE_CHILD, "11:00", "12:00", "completed"),
      ],
    },
    { laneId: PARENT, includeHistoricalStatuses: true },
  );

  const entry = result.entries.find((candidate) => candidate.id === "r-history");
  assert.equal(entry?.laneId, INACTIVE_CHILD);
  assert.equal(entry?.isHistorical, true);
  assert.equal(entry?.laneName, "Oś rodzic — Stanowisko historyczne");
  assert.equal(result.lanes.find((lane) => lane.id === INACTIVE_CHILD)?.isHistoricalOnly, true);
});
