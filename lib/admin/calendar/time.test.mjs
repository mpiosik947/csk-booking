...
const ACTIVE_LANE_ID = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa";
const OLD_LANE_ID = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";

function sampleRows() {
  return {
    lanes: [
      { id: ACTIVE_LANE_ID, name: "Oś 50 m", is_active: true, display_order: 10, booking_step_minutes: 60, resource_kind: "lane", parent_lane_id: null, whole_lane_bookable: true, positions_bookable: false, lane_booking_rules: { online_bookable: true } },
      { id: OLD_LANE_ID, name: "Stara oś", is_active: false, display_order: 90, booking_step_minutes: 60, resource_kind: "lane", parent_lane_id: null, whole_lane_bookable: true, positions_bookable: false, lane_booking_rules: { online_bookable: false } },
    ],
    reservations: [
      {
        id: "reservation-1",
...
test("calendar endpoint keeps authentication, RLS and read-only scope", async () => {
  ...
  assert.match(source, /"get_my_tenant_role_v1"/);
  assert.doesNotMatch(source, /SUPABASE_SERVICE_ROLE_KEY|event_registrations/);
  assert.doesNotMatch(source, /\.(?:insert|update|upsert)\(/);
  assert.doesNotMatch(source, /(?:laneRequest|reservationRequest|blockRequest|eventRequest)\.delete\(/);
});
