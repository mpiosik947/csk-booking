...
test("parent and one child remain one ordered resource family", () => {
  ...
  assert.deepEqual(
    model.map(({ displayName, depth }) => ({ displayName, depth })),
    [
      { displayName: "Oś główna", depth: 0 },
      { displayName: "Oś główna — Stanowisko 1", depth: 1 },
    ]
  );
});
...
test("parent and twenty children remain deterministic without a child limit", () => {
  ...
  const model = requireModel([...children, lane()]);

  assert.equal(model.length, 21);
  assert.equal(model[0].displayName, "Oś główna");
  assert.equal(model[1].displayName, "Oś główna — Stanowisko 1");
  assert.equal(model[20].displayName, "Oś główna — Stanowisko 20");
  assert.equal(model.slice(1).every((resource) => resource.depth === 1), true);
});
