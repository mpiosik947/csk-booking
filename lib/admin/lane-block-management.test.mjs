...
test("lane block cards are mobile-first, accessible, and status-filterable", async () => {
  ...
  assert.match(source, /id="lane-block-status-filter"/);
  assert.match(source, /visibleBlocks\.map/);
  assert.match(source, /flex min-w-0 flex-col gap-5 sm:flex-row/);
  assert.match(source, /break-words text-xl/);
  assert.match(source, /min-h-11 w-full[\s\S]*sm:w-auto/);
