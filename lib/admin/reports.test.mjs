...
test("report URL filters round-trip without PII", () => {
  const filters = {
    ...
    paymentStatus: "paid",
    bookingType: "single_position",
  };
  const params = buildReportSearchParams(filters);
  const parsed = parseReportFiltersFromSearchParams(params, "2026-09-05");
  assert.deepEqual(parsed, { ok: true, filters });
  assert.equal(params.has("email"), false);
  assert.equal(params.has("phone"), false);
  assert.equal(reportFiltersEqual(filters, parsed.ok && parsed.filters), true);
});

test("missing URL filters use the current day and invalid values fail closed", () => {
  assert.deepEqual(
    parseReportFiltersFromSearchParams(new URLSearchParams(), "2026-09-05"),
    {
      ok: true,
      filters: {
        startDate: "2026-09-05",
...
function exportRow(overrides = {}) {
  return {
    ...
    start_time: "10:00:00",
    end_time: "11:00:00",
    resource_label: "Oś 100 m — Stanowisko 1",
    booking_type: "single_position",
    reservation_status: "confirmed",
    payment_status: "paid",
    total_price: 70,
    ...overrides,
...
test("export parser accepts only the PII-minimal bounded contract", () => {
  const parsed = parseAdminReservationExport(exportPayload([exportRow()]));
  assert.equal(parsed?.ok, true);
  assert.deepEqual(parsed?.ok && Object.keys(parsed.export.rows[0]).sort(), [
    "bookingType",
    "endTime",
    "paymentStatus",
    "reservationDate",
...
test("CSV uses UTF-8 BOM, semicolons, CRLF and quotes special text", () => {
  const parsed = parseAdminReservationExport(
    exportPayload([
      exportRow({ resource_label: 'Oś; "A", linia\nDruga' }),
    ]),
  );
  assert.ok(parsed?.ok);
  const csv = buildAdminReservationCsv(parsed.export.rows);
  assert.equal(csv.startsWith("\uFEFF"), true);
  assert.match(csv, /"Oś; ""A"", linia\nDruga"/u);
  assert.equal(csv.endsWith("\r\n"), true);
  assert.equal(csv.split("\r\n").length, 3);
});

test("CSV neutralizes formula injection after leading whitespace", () => {
  const rows = ["=1+1", "+SUM(A1:A2)", "-2+3", "@cmd", "  =hidden"].map(
    (resourceLabel) => ({
      reservationDate: "2026-09-05",
      startTime: "10:00",
      endTime: "11:00",
      resourceLabel,
      bookingType: "whole_lane",
      reservationStatus: "confirmed",
      paymentStatus: "paid",
      totalPrice: 1,
    }),
  );
  const csv = buildAdminReservationCsv(rows);
  for (const dangerous of rows.map((row) => row.resourceLabel)) {
    assert.match(csv, new RegExp(`"'${dangerous.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`));
  }
  assert.doesNotMatch(csv, /customer|email|phone|address|token|admin_note/iu);
});

test("empty and 5000-row CSV exports are deterministic", () => {
  assert.equal(buildAdminReservationCsv([]).split("\r\n").length, 2);
  const row = {
    reservationDate: "2026-09-05",
    startTime: "10:00",
    endTime: "11:00",
    resourceLabel: "Żółta oś",
    bookingType: "whole_lane",
    reservationStatus: "confirmed",
    paymentStatus: "paid",
    totalPrice: 50,
  };
  const csv = buildAdminReservationCsv(Array.from({ length: 5000 }, () => row));
  assert.equal(csv.split("\r\n").length, 5002);
  assert.match(csv, /Żółta oś/u);
});
