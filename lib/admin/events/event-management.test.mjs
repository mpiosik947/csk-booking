...
test("40. maps every known RPC code to its exact Polish message", () => {
  const expected = {
    ...
    not_allowed: ["error", "Nie masz uprawnień do wykonania tej operacji."],
    invalid_input: ["error", "Sprawdź wprowadzone dane."],
    invalid_time_range: ["error", "Sprawdź godzinę rozpoczęcia i zakończenia wydarzenia."],
    outside_booking_hours: ["error", "Godziny wydarzenia wykraczają poza dostępny zakres dla wybranej osi."],
    invalid_lane: ["error", "Wybrana oś jest niedostępna."],
    inactive_lane: ["error", "Wybrana oś nie jest obecnie aktywna."],
    invalid_hierarchy: ["error", "Nie można wykonać operacji z powodu nieprawidłowej konfiguracji osi."],
    reservation_conflict: ["error", "Nie można zapisać wydarzenia, ponieważ w tym czasie istnieje rezerwacja."],
    lane_block_conflict: ["error", "Nie można zapisać wydarzenia, ponieważ wybrana oś jest w tym czasie zablokowana."],
    event_conflict: ["error", "Nie można zapisać wydarzenia, ponieważ w tym czasie istnieje inne wydarzenie."],
    event_not_found: ["error", "Nie znaleziono tego wydarzenia."],
    internal_error: ["error", "Nie udało się wykonać operacji. Spróbuj ponownie."],
  };
...
test("41. adds a locally known lane name to conflict messages", () => {
  assert.deepEqual(
    ...
    { kind: "error", message: "Nie można zapisać wydarzenia, ponieważ w tym czasie istnieje rezerwacja na osi: Oś 100 m." }
  );
});
...
test("42. hides an unknown conflict UUID", () => {
  ...
  assert.deepEqual(result, { kind: "error", message: "Nie można zapisać wydarzenia, ponieważ wybrana oś jest w tym czasie zablokowana." });
...
test("43. maps an unknown code to a controlled error", () => {
  assert.equal(
    validateEventRpcResult(rpcResult("future_code")).code,
...
test("67. sorts events by date and start time in both directions", () => {
  ...
  assert.deepEqual(
    sortAdminEvents(events, "latest").map((event) => event.id),
    [sameDayLate.id, sameDayEarly.id, early.id]
  );
  assert.deepEqual(
    sortAdminEvents(events, "nearest").map((event) => event.id),
...
test("68. event sorting is deterministic and does not mutate its input", () => {
  ...
  const before = [...events];
  const sorted = sortAdminEvents(events, "latest");
...
test("69. filters active, hidden, and all events without mutating input", () => {
  const active = normalizeAdminEvent(eventRow({ is_active: true })).value;
  const hidden = normalizeAdminEvent(eventRow({
    id: "22222222-2222-4222-8222-222222222222",
    is_active: false,
  })).value;
  const events = [active, hidden];
  const before = [...events];

  assert.deepEqual(filterAdminEvents(events, "active"), [active]);
  assert.deepEqual(filterAdminEvents(events, "hidden"), [hidden]);
  assert.deepEqual(filterAdminEvents(events, "all"), events);
  assert.notEqual(filterAdminEvents(events, "all"), events);
  assert.deepEqual(events, before);
});

test("70. filters before sorting without changing the source events", () => {
  const activeLater = normalizeAdminEvent(eventRow({
    id: "22222222-2222-4222-8222-222222222222",
  ...
  const activeSooner = normalizeAdminEvent(eventRow({
    id: "44444444-4444-4444-8444-444444444444",
    event_date: "2026-09-10",
    is_active: true,
  })).value;
  const events = [activeLater, hidden, activeSooner];
  const before = [...events];

  assert.deepEqual(
    sortAdminEvents(filterAdminEvents(events, "active"), "nearest").map(
      (event) => event.id
    ),
    [activeSooner.id, activeLater.id]
  );
  assert.deepEqual(events, before);
});

test("71. accepts every V2 result code with its exact safe shape", () => {
  const successCodes = ["created", "updated", "activated", "deactivated"];
  const plainErrorCodes = [
    "not_allowed",
    "invalid_input",
    "invalid_time_range",
  ...
  for (const code of successCodes) {
    assert.equal(validateEventRpcResult(rpcResult(code)).ok, true);
  }

  assert.equal(
    validateEventRpcResult(rpcResult("no_change", { changed: false })).ok,
    true
  );

  for (const code of plainErrorCodes) {
    assert.equal(
      validateEventRpcResult(
        rpcResult(code, { ok: false, changed: false })
      ).ok,
      true
    );
  }
  ...
  for (const [code, conflictType] of [
...
test("72. active parent and future active position use hierarchy display order", () => {
  const parent = lane(LANE_A, "Oś rodzic", 30).shooting_lanes;
  const child = {
    ...lane(LANE_B, "Stanowisko", 1).shooting_lanes,
    resource_kind: "position",
    parent_lane_id: LANE_A,
  };
  const normalized = normalizeActiveEventLanes([child, parent]);

  assert.ok(normalized);
  assert.deepEqual(
    normalized.map((item) => item.displayName),
    ["Oś rodzic", "Oś rodzic — Stanowisko"]
  );
  assert.equal(normalized[1].depth, 1);
  assert.equal(normalized[1].isPosition, true);
});

test("73. inactive positions stay outside the active event selector", () => {
  const parent = lane(LANE_A, "Oś rodzic", 30).shooting_lanes;
  const inactiveChild = {
    ...lane(LANE_B, "Stanowisko", 1).shooting_lanes,
    resource_kind: "position",
  ...
  assert.equal(normalizeActiveEventLanes([parent, inactiveChild]), null);
});

test("74. event lane relations render a position with its parent label", () => {
  const parent = lane(LANE_A, "Oś rodzic", 30).shooting_lanes;
  const child = {
    ...lane(LANE_B, "Stanowisko", 1).shooting_lanes,
    resource_kind: "position",
  ...
  const normalized = normalizeAdminEvent(
    eventRow({
      event_lanes: [{ lane_id: LANE_B, shooting_lanes: child }],
    })
  );

  assert.equal(normalized.ok, true);
  assert.equal(normalized.value.lanes[0].displayName, "Oś rodzic — Stanowisko");
});
