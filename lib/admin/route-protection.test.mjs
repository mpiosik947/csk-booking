...
test("SEC-011 protects every current admin page with its exact role matrix", () => {
  ...
  assert.deepEqual(getAdminRouteRoles("/admin/settings"), ["admin"]);
...
test("middleware applies authentication and active tenant membership before route access", async () => {
  const source = await readFile(middlewarePath, "utf8");
  const authIndex = source.indexOf("supabase.auth.getUser()");
  const tenantIndex = source.indexOf('"resolve_active_tenant_by_slug_v1"');
  const roleIndex = source.indexOf('"get_my_tenant_role_v1"');
  const accessIndex = source.indexOf("canRoleAccessAdminRoute(path, role)");

  assert.match(source, /matcher: \["\/\(\(\?!_next\/static/);
  assert.ok(source.indexOf('request.headers.get("host")') < authIndex);
  assert.match(source, /if \(userError \|\| !user\)/);
  assert.match(source, /tenantError \|\| !tenant/);
  assert.match(source, /ADMIN_STAFF_ROLES\.includes\(role\)/);
  assert.ok(authIndex >= 0 && authIndex < tenantIndex);
  assert.ok(tenantIndex < roleIndex && roleIndex < accessIndex);
  assert.doesNotMatch(source, /\.from\("profiles"\)|get_my_role/);
...
test("the /api/admin route keeps independent fail-closed server authorization", async () => {
  ...
  const tenantIndex = source.indexOf("resolvePublicTenantContext(supabase, requestedSlug)");
  const roleIndex = source.indexOf('"get_my_tenant_role_v1"');
  ...
  assert.match(source, /parseCalendarFeedRole\(roleData === "employee"/);
  assert.ok(authIndex >= 0 && authIndex < roleIndex);
  assert.ok(authIndex < tenantIndex && tenantIndex < roleIndex);
