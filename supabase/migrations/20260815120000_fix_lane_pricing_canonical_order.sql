-- Fix canonical display_order assignment for newly inserted family pricing rows.
-- Existing pricing data is intentionally not rewritten. The read path validates active
-- ranges independently, while the next controlled family write canonicalizes its target.
...
comment on function public.admin_set_lane_booking_family_configuration_v2(uuid,bigint,jsonb,boolean) is
  'Atomically replaces one complete lane-family target with optimistic concurrency, controlled confirmation and canonical pricing order.';
