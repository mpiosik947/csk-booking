...
begin
  ...
  perform pg_temp.ok(45,'single-active-tenant rollout guard is retired',pg_catalog.to_regclass('public.tenants_single_active_runtime_guard') is null,'Active tenant guard remains.');
  ...
  perform pg_temp.ok(59,'critical profile RPCs remain SECURITY DEFINER',not exists(select 1 from pg_catalog.unnest(array['admin_list_users_v2','get_reservation_customer_profiles_v1','update_my_profile_v2']::text[]) expected(name) where not exists(select 1 from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.proname=expected.name and procedure.prosecdef)),'Profile definer inventory differs.');
  perform pg_temp.ok(60,'9D-1 and 9D-4B-1A profile readers are membership-aware',exists(select 1 from pg_catalog.pg_proc where proname='get_reservation_customer_profiles_v1' and prosrc~'\mget_my_tenant_role_v1\M') and exists(select 1 from pg_catalog.pg_proc where proname='admin_list_users_v2' and prosrc~'\mget_my_tenant_role_v1\M' and prosrc~'\mtenant_user_admin_notes\M'),'Expected phased profile RPC boundary changed.');
  perform pg_temp.ok(61,'lane configuration execution modes reflect 9D-3C',exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_create_lane_booking_family_v2' and procedure.prosecdef) and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_family_configuration_v2' and procedure.prosecdef) and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_configuration' and not procedure.prosecdef),'Lane writer boundary differs.');
  perform pg_temp.ok(62,'current lane family entry points are tenant-scoped and legacy cores remain non-client invokers',exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_create_lane_booking_family_v2__saas9ec2b_core' and procedure.prosrc~'\mget_my_tenant_role_v1\M' and not procedure.prosecdef) and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_family_configuration_v3' and procedure.prosrc~'\mget_my_tenant_role_v1\M' and procedure.prosecdef) and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_configuration' and not procedure.prosecdef),'Expected final lane boundary changed.');
  ...
  perform pg_temp.ok(64,'compatibility defaults and second-tenant rollout guard retired',(select pg_catalog.count(*)=0 from information_schema.columns where table_schema='public' and table_name in('shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations','email_deliveries') and column_name='tenant_id' and column_default is not null) and pg_catalog.to_regclass('public.tenants_single_active_runtime_guard') is null,'Compatibility retirement or guard changed.');
end;
$tests$;
