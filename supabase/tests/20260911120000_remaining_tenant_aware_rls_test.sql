...
begin
  ...
  perform pg_temp.ok(62,'9D-3B hardens family create while 3C writers remain blockers',exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_create_lane_booking_family_v1' and procedure.prosrc~'\mget_my_tenant_role_v1\M') and not exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname in('admin_set_lane_booking_configuration','admin_set_lane_booking_family_configuration_v2') and procedure.prosrc~'\m(is_tenant_member_v1|has_tenant_role_v1|get_my_tenant_role_v1)\M'),'Expected phased lane definer boundary changed.');
