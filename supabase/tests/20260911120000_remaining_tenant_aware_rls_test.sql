...
begin
  ...
  perform pg_temp.ok(61,'lane configuration execution modes reflect 9D-3C',exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_create_lane_booking_family_v1' and procedure.prosecdef) and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_family_configuration_v2' and procedure.prosecdef) and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_configuration' and not procedure.prosecdef),'Lane writer boundary differs.');
  perform pg_temp.ok(62,'9D-3B create and 9D-3C family update are membership-aware',exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_create_lane_booking_family_v1' and procedure.prosrc~'\mget_my_tenant_role_v1\M') and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_family_configuration_v2__saas9d3c_core' and not procedure.prosecdef and procedure.prosrc~'\mget_my_tenant_role_v1\M') and exists(select 1 from pg_catalog.pg_proc procedure where procedure.proname='admin_set_lane_booking_configuration' and not procedure.prosecdef),'Expected phased lane boundary changed.');
