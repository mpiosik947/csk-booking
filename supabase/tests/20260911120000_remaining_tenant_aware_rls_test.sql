...
begin
  ...
  perform pg_temp.ok(60,'legacy profile RPCs remain explicit 9D blockers',not exists(select 1 from pg_catalog.pg_proc where proname in('admin_list_users_v1','get_reservation_customer_profiles_v1') and prosrc~'\m(is_tenant_member_v1|has_tenant_role_v1|get_my_tenant_role_v1)\M'),'Expected legacy profile bypass changed.');
