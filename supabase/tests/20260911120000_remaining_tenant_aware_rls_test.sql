...
begin
  ...
  perform pg_temp.ok(64,'compatibility defaults retired and second-tenant guard unchanged',(select pg_catalog.count(*)=0 from information_schema.columns where table_schema='public' and table_name in('shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations','email_deliveries') and column_name='tenant_id' and column_default is not null) and pg_catalog.to_regclass('public.tenants_single_active_runtime_guard') is not null,'Compatibility retirement or guard changed.');
end;
$tests$;
