...
begin
  ...
  perform pg_temp.ok(5,'SECURITY DEFINER count is exactly 70 after 9E-A',(select count(*)=70 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
...
select case when
  (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=70
