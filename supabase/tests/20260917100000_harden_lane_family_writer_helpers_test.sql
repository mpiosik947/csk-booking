...
begin
  ...
  perform pg_temp.ok(5,'SECURITY DEFINER count is 75 after PRODUCT-10A public directory',(select count(*)=75 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
...
select case when
  (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=75
