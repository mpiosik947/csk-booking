...
begin
  ...
  perform pg_temp.ok(5,'SECURITY DEFINER count is exactly 76 after 9E-C1',(select count(*)=76 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
  ...
  result:=pg_temp.as_actor_json(admin_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Root A',jsonb_build_array(pg_temp.resource_payload(marker||' Child A',2)))));
...
select case when
  (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=76
