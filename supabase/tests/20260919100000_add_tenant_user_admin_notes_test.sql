...
begin
  ...
  perform pg_temp.ok(32,'SECURITY DEFINER count is 85 after PRODUCT-10C public landing',(select pg_catalog.count(*) from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef)=  85,'definer count differs');
