...
begin
  ...
  perform pg_temp.ok(32,'SECURITY DEFINER count is 69 after 9D-4B-2B',(select pg_catalog.count(*) from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef)=69,'definer count differs');
  ...
  perform pg_temp.ok(34,'account-wide lifecycle remains separate',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.export_my_data_v1()'::regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='ffa6b35c5502a347e463110401032061' and pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.anonymize_my_account_v1()'::regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='7e4d950e75e6e5782b139f11269d03a0','account lifecycle drifted');
