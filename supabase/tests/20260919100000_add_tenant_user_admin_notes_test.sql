...
begin
  ...
  perform pg_temp.ok(32,'SECURITY DEFINER count is 80 after PRODUCT-10C public landing',(select pg_catalog.count(*) from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef)=80,'definer count differs');
