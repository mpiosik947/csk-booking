...
begin
 ...
 perform pg_temp.ok(2,'100 DEFINER and zero defaults',(select count(*)=100 from pg_proc p where p.pronamespace='public'::regnamespace and p.prosecdef) and (select count(*)=0 from information_schema.columns where table_schema='public' and table_name in ('shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations','email_deliveries') and column_name='tenant_id' and column_default='''c5c00000-0000-4000-8000-000000000001''::uuid'),'inventory');
