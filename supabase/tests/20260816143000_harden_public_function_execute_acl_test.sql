...
begin
  ...
  perform pg_temp.record_result(1,'Complete public function inventory',
    (select pg_catalog.count(*)=68 from pg_temp.expected_function_acl)
    and v_actual_count=68
    and not exists(
    ...
    'The exact 68-function inventory has no missing or unexpected signature.');
  ...
  perform pg_temp.record_result(3,'Exact anon ACL matrix',
    ...
    and (select pg_catalog.count(*)=3 from pg_temp.expected_function_acl where anon_execute),
    'anon can execute only the three intended non-PII public readers.');
  ...
  perform pg_temp.record_result(4,'Exact authenticated ACL matrix',
    ...
    and (select pg_catalog.count(*)=43 from pg_temp.expected_function_acl where authenticated_execute),
    'authenticated has exactly the 43 user, policy-helper and internally authorized RPC grants.');
  ...
  perform pg_temp.record_result(8,'Application function creator scope is exact',
    ...
    'All 68 application functions are owned by postgres, whose public-schema defaults are hardened.');
