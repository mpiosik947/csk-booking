...
begin
  ...
  perform pg_temp.record_result(1,'Complete public function inventory',
    (select pg_catalog.count(*)=58 from pg_temp.expected_function_acl)
    and v_actual_count=58
    and not exists(
    ...
    'The exact 58-function inventory has no missing or unexpected signature.');
  ...
  perform pg_temp.record_result(3,'Exact anon ACL matrix',
    ...
    and (select pg_catalog.count(*)=1 from pg_temp.expected_function_acl where anon_execute),
    'anon can execute only get_public_booking_configuration_v1().');
  ...
  perform pg_temp.record_result(4,'Exact authenticated ACL matrix',
    ...
    and (select pg_catalog.count(*)=36 from pg_temp.expected_function_acl where authenticated_execute),
    'authenticated has exactly the 36 user, policy-helper and internally authorized RPC grants.');
  ...
  perform pg_temp.record_result(8,'Application function creator scope is exact',
    ...
    'All 58 application functions are owned by postgres, whose public-schema defaults are hardened.');
  ...
  perform pg_temp.record_result(11,'SECURITY DEFINER ownership and search_path',
    ...
    'All 52 SECURITY DEFINER functions remain postgres-owned with an explicit search_path.');
