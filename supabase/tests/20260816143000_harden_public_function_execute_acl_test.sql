...
insert into expected_function_acl values
  -- PRODUCT-10C: public presentation DTO and tenant-admin settings contracts.
  ('public.admin_get_tenant_public_settings_v1(text)','C',false,true,false),
  ('public.admin_update_tenant_public_settings_v1(text,jsonb,timestamp with time zone)','C',false,true,false),
  ('public.get_public_tenant_landing_v2(text)','B',true,true,false),
  -- PRODUCT-10B: public URL selector and PII-free tenant landing contracts.
  ('public.enforce_public_tenant_slug_namespace()','E',false,false,false),
  ('public.get_public_tenant_directory_v2(text)','B',true,true,false),
  ('public.get_public_tenant_landing_v1(text)','B',true,true,false),
  -- PRODUCT-10A: bounded, PII-free directory for active and explicitly published tenants.
  ('public.get_public_tenant_directory_v1(text)','B',true,true,false),
  -- SAAS-9F: authenticated, PII-free tenant selector for the global dashboard.
  ('public.get_my_active_tenants_v1()','B',false,true,false),
  -- SAAS-9E-C Phase 3: additive resource-scoped ICS and account writer.
  ('public.get_my_reservation_calendar_v1(uuid)','B',false,true,false),
  ('public.update_my_profile_v2(text,text,text,text,text,text,boolean,boolean,boolean,boolean,boolean,boolean,boolean,boolean,boolean,boolean)','B',false,true,false),
  -- SAAS-9E-C Phase 2: exact versioned RPC/closed-core inventory.
  ('public.admin_list_events_v2(uuid,text,text,text,integer,integer)','C',false,true,false),
  ('public.admin_create_event_v3(uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])','C',false,true,false),
  ('public.admin_create_event_v3__saas9ec2a_core(uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])','A',false,false,false),
  ...
  ('public.admin_get_lane_booking_configuration_v3__c2b_resource(uuid)','A',false,false,false),
  ('public.admin_get_lane_booking_configuration_v3__saas9ec2b_core(uuid)','A',false,false,false),
  ('public.admin_create_lane_booking_family_v2__saas9ec2b_core(uuid,jsonb)','A',false,false,false),
  ('public.admin_get_lane_booking_configuration_v3(uuid)','C',false,true,false),
  ('public.admin_create_lane_booking_family_v2(uuid,jsonb)','C',false,true,false),
  ('public.admin_set_lane_booking_family_configuration_v3(uuid,uuid,bigint,jsonb,boolean)','C',false,true,false),
  ('public.admin_get_reservation_report_v3(uuid,date,date,uuid,text,text,text,integer,integer)','C',false,true,false),
  ('public.admin_get_reservation_report_export_v2(uuid,date,date,uuid,text,text,text)','C',false,true,false),
  ('public.admin_list_users_v2(uuid,integer,integer,text,text,text,text)','C',false,true,false),
  ('public.admin_set_user_role_v2(uuid,uuid,text)','C',false,true,false),
  ('public.admin_set_user_note_v2(uuid,uuid,text)','C',false,true,false),
  ...
  ('public.legacy_profile_role_to_tenant_role_v1(text)','A',false,false,false),
  ...
  ('public.mark_event_registration_paid(uuid)','C',false,true,false),
  ...
  ('public.self_onboard_tenant_v1(text)','B',false,true,false),
  ('public.redact_account_audit_details_v1(jsonb,uuid,text,text[])','A',false,false,false),
  ('public.resolve_active_tenant_by_slug_v1(text)','B',true,true,false),
...
begin
  ...
  perform pg_temp.record_result(1,'Complete public function inventory',
    (select pg_catalog.count(*)=136 from pg_temp.expected_function_acl)
    and v_actual_count=136
    and not exists(
    ...
    'The exact 136-function PRODUCT-10C inventory has no missing or unexpected signature.');
  ...
  perform pg_temp.record_result(3,'Exact anon ACL matrix',
    ...
    and (select pg_catalog.count(*)=10 from pg_temp.expected_function_acl where anon_execute),
    'anon can execute only the ten intended non-PII public readers/policy helpers.');
  ...
  perform pg_temp.record_result(4,'Exact authenticated ACL matrix',
    ...
    and (select pg_catalog.count(*)=68 from pg_temp.expected_function_acl where authenticated_execute),
    'authenticated has exactly the 68 current tenant-aware RPC grants.');
  ...
  perform pg_temp.record_result(5,'Exact service_role ACL matrix',
    ...
    and (select pg_catalog.count(*)=4 from pg_temp.expected_function_acl where service_role_execute),
    'service_role retains only the four explicitly intended server and rollback grants.');

  perform pg_temp.record_result(6,'Trigger functions and internal helpers are isolated',
    (select pg_catalog.count(*)=11 from pg_temp.expected_function_acl where category='E')
    and not exists(
      select 1 from pg_temp.expected_function_acl expected
    ...
    and (select pg_catalog.count(distinct trigger_record.tgfoid)=11
  ...
  perform pg_temp.record_result(8,'Application function creator scope is exact',
    ...
    'All 136 application functions are owned by postgres, whose public-schema defaults are hardened.');
