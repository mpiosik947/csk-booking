...
begin
  ...
  perform pg_temp.record_result(1,'Complete public function inventory',
    ...
    'The exact 129-function post-retirement inventory has no missing or unexpected signature.');
  ...
  perform pg_temp.record_result(4,'Exact authenticated ACL matrix',
    ...
    and (select pg_catalog.count(*)=62 from pg_temp.expected_function_acl where authenticated_execute),
    'authenticated has exactly the 62 current tenant-aware RPC grants.');
