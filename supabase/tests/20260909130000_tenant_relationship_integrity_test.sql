...
begin
  ...
  perform pg_temp.ok(43,'RLS outside approved membership and booking cutovers is unchanged',(select pg_catalog.md5(coalesce(pg_catalog.string_agg(pg_catalog.concat_ws('|',tablename,policyname,cmd,roles::text,qual,with_check),E'\n' order by tablename,policyname),''))='d3c02109fea9966d27cfaef2e481f738' from pg_catalog.pg_policies where schemaname='public' and tablename not in ('tenant_memberships','shooting_lanes','reservations','lane_blocks')),'RLS changed outside approved 9C phases.');
