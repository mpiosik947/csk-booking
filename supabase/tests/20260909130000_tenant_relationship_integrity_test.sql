...
begin
  ...
  insert into public.email_deliveries(tenant_id,id,message_type,record_id,recipient_user_id)
  values(v_csk,v_delivery_reservation,'reservation_confirmation',v_reservation,v_user);
  perform pg_temp.ok(18,'reservation delivery preserves explicit matching tenant',(select tenant_id=v_csk from public.email_deliveries where id=v_delivery_reservation),'Reservation delivery tenant differs.');
  ...
  insert into public.email_deliveries(tenant_id,id,message_type,record_id,recipient_user_id)
  values(v_csk,v_delivery_event,'event_registration_confirmation',v_registration,v_user);
  perform pg_temp.ok(20,'event registration delivery preserves explicit matching tenant',(select tenant_id=v_csk from public.email_deliveries where id=v_delivery_event),'Event delivery tenant differs.');
  ...
  perform pg_temp.ok(22,'missing delivery target is denied',pg_temp.raises(pg_catalog.format('insert into public.email_deliveries(tenant_id,id,message_type,record_id,recipient_user_id) values(%L,%L,%L,%L,%L)',v_csk,pg_catalog.gen_random_uuid(),'reservation_confirmation',pg_catalog.gen_random_uuid(),v_user),'23503'),'Missing delivery target was accepted.');
  perform pg_temp.ok(23,'unknown delivery type is denied',pg_temp.raises(pg_catalog.format('insert into public.email_deliveries(tenant_id,id,message_type,record_id,recipient_user_id) values(%L,%L,%L,%L,%L)',v_csk,pg_catalog.gen_random_uuid(),'unknown_type',v_reservation,v_user),'23514'),'Unknown delivery type was accepted.');
  ...
  update public.tenants set status='active' where id=v_other;
  perform pg_temp.ok(42,'multiple active tenants are supported after the readiness cutover',(select pg_catalog.count(*)=2 from public.tenants where status='active') and exists(select 1 from public.tenants where id=v_other and status='active'),'Second active tenant readiness regressed.');
  update public.tenants set status='dormant' where id=v_other;
  ...
  perform pg_temp.ok(45,'critical RPC fingerprints match the current tenant-aware contracts',pg_catalog.md5(replace(replace(pg_catalog.pg_get_functiondef('public.get_my_role()'::regprocedure),chr(13)||chr(10),chr(10)),chr(13),chr(10)))='eec66d2c695d3892caec4d4242756ed0' and pg_catalog.md5(replace(replace(pg_catalog.pg_get_functiondef('public.create_reservation_v2(uuid,date,time without time zone,integer,integer,uuid,text)'::regprocedure),chr(13)||chr(10),chr(10)),chr(13),chr(10)))='ff1c273379e9ee3af3a1a60d131af81b','Critical RPC changed.');
  ...
  perform pg_temp.ok(48,'explicit membership foundation and approved SAAS-9C RLS cutovers are active',not exists(select 1 from public.tenant_memberships membership where membership.user_id=v_user) and pg_catalog.to_regprocedure('public.self_onboard_tenant_v1(text)') is not null and pg_catalog.to_regprocedure('public.has_tenant_role_v1(uuid,text[])') is not null and not exists(select 1 from pg_catalog.pg_policies where schemaname='public' and tablename not in ('tenant_memberships','shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations','audit_logs','profiles','lane_booking_rules','lane_booking_durations','lane_pricing_rules') and (coalesce(qual,'')||coalesce(with_check,'')) ~ '(is_tenant_member_v1|has_tenant_role_v1|is_active_public_tenant_v1)'),'Global profile creation must not imply membership; explicit onboarding and approved RLS phase isolation must remain active.');
  perform pg_temp.ok(49,'seven temporary CSK defaults are retired',not exists(select 1 from information_schema.columns where table_schema='public' and column_name='tenant_id' and table_name in ('shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations','email_deliveries') and column_default is not null),'A compatibility tenant default remains.');
