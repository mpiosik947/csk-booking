...
begin
  ...
  perform pg_temp.ok(43,'RLS outside approved membership, booking, and Events cutovers is unchanged',(select pg_catalog.md5(coalesce(pg_catalog.string_agg(pg_catalog.concat_ws('|',tablename,policyname,cmd,roles::text,qual,with_check),E'\n' order by tablename,policyname),''))='4e56d58f87ebc27bf2995ffd6374031c' from pg_catalog.pg_policies where schemaname='public' and tablename not in ('tenant_memberships','shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations')),'RLS changed outside approved 9C phases.');
