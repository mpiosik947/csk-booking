...
begin
  ...
  perform pg_temp.ok(43,'business RLS fingerprint is unchanged',(select pg_catalog.md5(coalesce(pg_catalog.string_agg(pg_catalog.concat_ws('|',tablename,policyname,cmd,roles::text,qual,with_check),E'\n' order by tablename,policyname),''))='f5c428bd4e241af39f690c1aafcfad08' from pg_catalog.pg_policies where schemaname='public' and tablename<>'tenant_memberships'),'Business RLS policies changed.');
