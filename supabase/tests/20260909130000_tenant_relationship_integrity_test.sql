...
begin
  ...
  perform pg_temp.ok(43,'RLS policy fingerprint is unchanged',(select pg_catalog.md5(coalesce(pg_catalog.string_agg(pg_catalog.concat_ws('|',tablename,policyname,cmd,roles::text,qual,with_check),E'\n' order by tablename,policyname),''))='f5c428bd4e241af39f690c1aafcfad08' from pg_catalog.pg_policies where schemaname='public'),'RLS policies changed.');
  perform pg_temp.ok(44,'table ACL fingerprint is unchanged',(select pg_catalog.md5(coalesce(pg_catalog.string_agg(relation.relname||'|'||coalesce(relation.relacl::text,''),E'\n' order by relation.relname),''))='cc439ed94c9949ad6461925f428940a6' from pg_catalog.pg_class relation join pg_catalog.pg_namespace namespace on namespace.oid=relation.relnamespace where namespace.nspname='public' and relation.relkind in ('r','p')),'Table ACL changed.');
  ...
  perform pg_temp.ok(48,'tenant memberships remain dormant',(select count(*)=0 from public.tenant_memberships) and not exists(select 1 from pg_catalog.pg_proc function_record join pg_catalog.pg_namespace namespace on namespace.oid=function_record.pronamespace where namespace.nspname='public' and function_record.prokind='f' and pg_catalog.pg_get_functiondef(function_record.oid) like '%tenant_memberships%' and function_record.proname not in ('set_updated_at')),'Membership authorization was activated.');
...
begin
  if (select count(*) from pg_temp.test_results)<>49 then raise exception 'SAAS-9B-3 expected 49 transactional checks.'; end if;
...
select 'ok 50 - rollback leaves zero SAAS-9B-3 fixture';
