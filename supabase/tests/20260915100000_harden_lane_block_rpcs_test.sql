...
begin
  ...
  perform pg_temp.ok(33,'SECURITY DEFINER inventory includes PRODUCT-10B public readers',(select count(*)=77 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count drifted.');
  perform pg_temp.ok(34,'Booking and Events public contract fingerprints are unchanged',md5(replace(replace(pg_get_functiondef('public.get_public_booking_configuration_v2(uuid)'::regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='c22681300c18658a77e94b58320c5986' and md5(replace(replace(pg_get_functiondef('public.get_public_event_list_v3(uuid,text,text,integer,integer)'::regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='5c455312be9f3b6a2e9bd26fc15ded0a','Unrelated public contract drifted.');
