...
begin
  ...
  perform pg_temp.ok(33,'SECURITY DEFINER inventory remains at approved post-9D-4B-2B count',(select count(*)=69 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count drifted.');
  perform pg_temp.ok(34,'Booking and Events public contract fingerprints are unchanged',md5(replace(replace(pg_get_functiondef('public.get_public_booking_configuration_v1()'::regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='2aee39e3d37d3d1a19f58c3626aa0365' and md5(replace(replace(pg_get_functiondef('public.get_public_event_list_v2(text,text,integer,integer)'::regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='642b84c0d78066e2071a0f0df1ce97ff','Unrelated public contract drifted.');
