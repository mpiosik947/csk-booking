...
begin
  ...
  update public.tenant_memberships set status='pending' where tenant_id=csk and user_id=pending_a;
  update public.tenant_memberships set status='suspended' where tenant_id=csk and user_id=suspended_a;
  ...
  perform pg_temp.ok(1,'exact three 9D-3B signatures remain',(select count(*)=3 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('admin_create_lane_booking_family_v1','admin_get_lane_booking_configuration_v1','admin_get_lane_booking_configuration_v2')),'Target inventory differs.');
  ...
  perform pg_temp.ok(5,'SECURITY DEFINER count is 69 after 9D-4B-2B',(select count(*)=69 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count drifted.');
  ...
  perform pg_temp.ok(11,'V1 cannot be called directly by authenticated',pg_temp.role_denied('authenticated','select public.admin_get_lane_booking_configuration_v1()'),'V1 remained client-callable.');
  ...
  result:=pg_temp.as_actor_json(admin_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Root A',positions)));
  ...
  perform pg_temp.ok(18,'employee remains denied create',(pg_temp.as_actor_json(employee_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Employee'))))->>'code'='not_allowed','Employee scope widened.');
  perform pg_temp.ok(19,'instructor and user remain denied create',(pg_temp.as_actor_json(instructor_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Instructor'))))->>'code'='not_allowed' and (pg_temp.as_actor_json(user_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' User'))))->>'code'='not_allowed','Role scope widened.');
  perform pg_temp.ok(20,'global admin without membership is denied create and read',(pg_temp.as_actor_json(global_admin,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Global'))))->>'code'='not_allowed' and pg_temp.reader_denied(global_admin,'select public.admin_get_lane_booking_configuration_v2()'),'Global role bypass remains.');
  perform pg_temp.ok(21,'pending membership is denied create and read',(pg_temp.as_actor_json(pending_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Pending'))))->>'code'='not_allowed' and pg_temp.reader_denied(pending_a,'select public.admin_get_lane_booking_configuration_v2()'),'Pending membership authorized.');
  perform pg_temp.ok(22,'suspended membership is denied create and read',(pg_temp.as_actor_json(suspended_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Suspended'))))->>'code'='not_allowed' and pg_temp.reader_denied(suspended_a,'select public.admin_get_lane_booking_configuration_v2()'),'Suspended membership authorized.');
  perform pg_temp.ok(23,'Tenant B member cannot be redirected into active Tenant A',(pg_temp.as_actor_json(admin_b,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' B Attempt'))))->>'code'='not_allowed' and pg_temp.reader_denied(admin_b,'select public.admin_get_lane_booking_configuration_v2()'),'Tenant B member crossed bridge authority.');

  result:=pg_temp.as_actor_json(admin_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Spoof')||jsonb_build_object('tenant_id',tenant_b)));
  ...
  perform pg_temp.ok(27,'anon and service cannot execute exposed 3B RPCs',pg_temp.role_denied('anon','select public.admin_get_lane_booking_configuration_v2()') and pg_temp.role_denied('service_role','select public.admin_create_lane_booking_family_v1(''{}''::jsonb)'),'Privileged ACL widened.');

  update public.tenants set status='dormant' where id=csk;
  perform pg_temp.ok(28,'zero active tenants fail creator closed',(pg_temp.as_actor_json(admin_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Zero'))))->>'code'='not_allowed','Zero-active creator did not fail closed.');
  perform pg_temp.ok(29,'zero active tenants fail reader closed',pg_temp.reader_denied(admin_a,'select public.admin_get_lane_booking_configuration_v2()'),'Zero-active reader did not fail closed.');
  ...
  perform pg_temp.ok(30,'two active tenants fail creator closed',(pg_temp.as_actor_json(admin_a,format('select public.admin_create_lane_booking_family_v1(%L::jsonb)',pg_temp.family_payload(marker||' Two'))))->>'code'='not_allowed','Two-active creator selected a tenant.');
  perform pg_temp.ok(31,'two active tenants fail reader closed',pg_temp.reader_denied(admin_a,'select public.admin_get_lane_booking_configuration_v2()'),'Two-active reader selected a tenant.');
...
select case when
  (select count(*) from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef)=69
