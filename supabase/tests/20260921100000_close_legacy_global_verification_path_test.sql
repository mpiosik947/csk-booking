...
begin
  perform pg_temp.ok(1,'target signatures exist',pg_catalog.to_regprocedure('public.update_profile_verification(uuid,text,text)') is not null and pg_catalog.to_regprocedure('public._apply_tenant_user_verification_v1(uuid,uuid,text,text,text,uuid)') is not null);
  perform pg_temp.ok(2,'SECURITY DEFINER count remains 69',(select pg_catalog.count(*)=69 from pg_catalog.pg_proc procedure_record join pg_catalog.pg_namespace namespace_record on namespace_record.oid=procedure_record.pronamespace where namespace_record.nspname='public' and procedure_record.prosecdef));
  perform pg_temp.ok(3,'compatibility defaults remain 7/7',(select pg_catalog.count(*)=7 from information_schema.columns where table_schema='public' and table_name in('shooting_lanes','reservations','lane_blocks','events','event_lanes','event_registrations','email_deliveries') and column_name='tenant_id' and column_default='''c5c00000-0000-4000-8000-000000000001''::uuid'));
  perform pg_temp.ok(4,'frozen profile trigger fingerprint unchanged',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.prevent_non_admin_profile_privilege_changes()'::regprocedure),pg_catalog.chr(13)||pg_catalog.chr(10),pg_catalog.chr(10)),pg_catalog.chr(13),pg_catalog.chr(10)))='d28cb697d8355a5e8005296a03ad63ea');
  perform pg_temp.ok(5,'legacy writer ACL is authenticated only',pg_catalog.has_function_privilege('authenticated','public.update_profile_verification(uuid,text,text)','EXECUTE') and not pg_catalog.has_function_privilege('public','public.update_profile_verification(uuid,text,text)','EXECUTE') and not pg_catalog.has_function_privilege('anon','public.update_profile_verification(uuid,text,text)','EXECUTE') and not pg_catalog.has_function_privilege('service_role','public.update_profile_verification(uuid,text,text)','EXECUTE'));
  ...
  perform pg_temp.ok(7,'target metadata is preserved',(select procedure_record.prosecdef and pg_catalog.pg_get_userbyid(procedure_record.proowner)='postgres' and procedure_record.proconfig=array['search_path=pg_catalog, public, pg_temp'] from pg_catalog.pg_proc procedure_record where procedure_record.oid='public.update_profile_verification(uuid,text,text)'::regprocedure));
  ...
  perform pg_temp.ok(10,'generic writer is admin-only and profile-role free',pg_catalog.strpos((select prosrc from pg_catalog.pg_proc where oid='public.update_profile_verification(uuid,text,text)'::regprocedure),'v_actor_role is distinct from ''admin''')>0 and pg_catalog.strpos((select prosrc from pg_catalog.pg_proc where oid='public.update_profile_verification(uuid,text,text)'::regprocedure),'profiles.role')=0 and pg_catalog.strpos((select prosrc from pg_catalog.pg_proc where oid='public.update_profile_verification(uuid,text,text)'::regprocedure),'''employee''')=0);
  ...
  result:=pg_temp.as_actor_text(admin_a,format('public.update_profile_verification(%L,''verify'',''A verified'')',user_x))::jsonb;
  ...
  perform pg_temp.ok(18,'employee generic writer is denied',pg_temp.as_actor_raises(employee_a,format('select public.update_profile_verification(%L,''mark_pending'',null)',user_x)));
  perform pg_temp.ok(19,'global profile admin without membership is denied',pg_temp.as_actor_raises(global_admin,format('select public.update_profile_verification(%L,''verify'',null)',user_x)));
  perform pg_temp.ok(20,'pending membership is denied',pg_temp.as_actor_raises(pending_admin,format('select public.update_profile_verification(%L,''verify'',null)',user_x)));
  perform pg_temp.ok(21,'suspended membership is denied',pg_temp.as_actor_raises(suspended_admin,format('select public.update_profile_verification(%L,''verify'',null)',user_x)));
  perform pg_temp.ok(22,'unrelated global user is denied',pg_temp.as_actor_raises(admin_a,format('select public.update_profile_verification(%L,''verify'',null)',unrelated)));
  perform pg_temp.ok(23,'foreign tenant admin is denied',pg_temp.as_actor_raises(admin_b,format('select public.update_profile_verification(%L,''verify'',null)',user_x)));
  ...
  perform pg_temp.ok(29,'active readers contain no profile verification fallback',pg_catalog.strpos((select prosrc from pg_catalog.pg_proc where oid='public.admin_list_users_v1(integer,integer,text,text,text,text)'::regprocedure),'profile.verification_status')=0 and pg_catalog.strpos((select prosrc from pg_catalog.pg_proc where oid='public.get_reservation_customer_profiles_v1(uuid[])'::regprocedure),'profile.verification_status')=0);
  ...
  perform pg_temp.ok(31,'account lifecycle functions remain unchanged',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.export_my_data_v1()'::regprocedure),pg_catalog.chr(13)||pg_catalog.chr(10),pg_catalog.chr(10)),pg_catalog.chr(13),pg_catalog.chr(10)))='ffa6b35c5502a347e463110401032061' and pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.anonymize_my_account_v1()'::regprocedure),pg_catalog.chr(13)||pg_catalog.chr(10),pg_catalog.chr(10)),pg_catalog.chr(13),pg_catalog.chr(10)))='7e4d950e75e6e5782b139f11269d03a0');
  perform pg_temp.ok(32,'Admin Users reader remains unchanged',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.admin_list_users_v1(integer,integer,text,text,text,text)'::regprocedure),pg_catalog.chr(13)||pg_catalog.chr(10),pg_catalog.chr(10)),pg_catalog.chr(13),pg_catalog.chr(10)))='bf37ec48de512ea45f5d4592df5f4eac');
  ...
  perform pg_temp.ok(34,'resource writer remains unchanged',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.update_reservation_customer_verification_v1(uuid,text,text)'::regprocedure),pg_catalog.chr(13)||pg_catalog.chr(10),pg_catalog.chr(10)),pg_catalog.chr(13),pg_catalog.chr(10)))='b4916056e23c1043baf17cbdab55caeb');
  perform pg_temp.ok(35,'owner reader remains unchanged',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.get_my_active_tenant_verification_v1()'::regprocedure),pg_catalog.chr(13)||pg_catalog.chr(10),pg_catalog.chr(10)),pg_catalog.chr(13),pg_catalog.chr(10)))='951c8262bc118089d8398282e1a95ce8');
