...
begin
  ...
  perform pg_temp.ok(39,'SECURITY DEFINER inventory includes 9E-A resolver',(select pg_catalog.count(*)=94 from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef),'Unexpected definer drift exists.');
