...
begin
  ...
  perform pg_temp.ok(39,'SECURITY DEFINER inventory reflects the approved 2C-1 and 2C-2 invoker conversions',(select pg_catalog.count(*)=70 from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef),'Unexpected definer drift exists.');
