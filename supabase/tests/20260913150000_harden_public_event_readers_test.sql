...
begin
  ...
  perform pg_temp.ok(39,'SECURITY DEFINER inventory remains 73',(select pg_catalog.count(*)=73 from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef),'Unexpected definer drift exists.');
