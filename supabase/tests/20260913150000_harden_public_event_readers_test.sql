...
begin
  ...
  perform pg_temp.ok(39,'SECURITY DEFINER inventory reflects approved hardening through 9D-3C',(select pg_catalog.count(*)=67 from pg_catalog.pg_proc procedure join pg_catalog.pg_namespace namespace on namespace.oid=procedure.pronamespace where namespace.nspname='public' and procedure.prosecdef),'Unexpected definer drift exists.');
