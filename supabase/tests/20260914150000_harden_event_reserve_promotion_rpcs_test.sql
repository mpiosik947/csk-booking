...
begin
  ...
  perform pg_temp.ok(7,'SECURITY DEFINER count is 77 after PRODUCT-10B public landing',(select pg_catalog.count(*)=77 from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
