...
begin
  ...
  perform pg_temp.ok(7,'SECURITY DEFINER count is 74 after the 9F tenant selector',(select pg_catalog.count(*)=74 from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
