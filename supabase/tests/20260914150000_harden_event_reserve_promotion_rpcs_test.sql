...
begin
  ...
  perform pg_temp.ok(7,'SECURITY DEFINER count is 69 after 9D-4B-2B',(select pg_catalog.count(*)=69 from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
