...
begin
  ...
  values(csk,user_occupied,'user','active'),
        (csk,user_first,'user','active'),
        (csk,user_second,'user','active')
  ...
  perform pg_temp.ok(6,'target normalized fingerprints include PRODUCT-10D prepare gate',pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.prepare_event_reserve_promotions(uuid)'::pg_catalog.regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='2ff6d2638878cf68166b970a9c42df47' and pg_catalog.md5(pg_catalog.replace(pg_catalog.replace(pg_catalog.pg_get_functiondef('public.complete_event_reserve_promotion(uuid,uuid,boolean,text)'::pg_catalog.regprocedure),E'\r\n',E'\n'),E'\r',E'\n'))='2c78ac26c5c55df3aac54360b610d39b','Target body differs.');
  perform pg_temp.ok(7,'SECURITY DEFINER count is 100 after PRODUCT-10C public landing',(select pg_catalog.count(*)=  104 from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'Definer count differs.');
