...
begin
  ...
  perform pg_temp.ok(2,'SECURITY DEFINER count is 69',(select count(*)=69 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef));
  ...
  perform pg_temp.ok(7,'frozen trigger fingerprint unchanged',md5(replace(replace(pg_get_functiondef('public.prevent_non_admin_profile_privilege_changes()'::regprocedure),chr(13)||chr(10),chr(10)),chr(13),chr(10)))='d28cb697d8355a5e8005296a03ad63ea');
  ...
  perform pg_temp.ok(19,'legacy generic writer denies unrelated user',pg_temp.as_actor_raises(admin_a,format('select public.update_profile_verification(%L,''verify'',null)',unrelated)));
  ...
  perform pg_temp.ok(22,'owner reader excludes note and actor fields',(select proargnames=array['verification_status','permissions_verified','permissions_verified_at','updated_at'] from pg_proc where oid='public.get_my_active_tenant_verification_v1()'::regprocedure));
  perform pg_temp.ok(23,'owner reader signature has four output fields',(select cardinality(proallargtypes)-pronargs=4 from pg_proc where oid='public.get_my_active_tenant_verification_v1()'::regprocedure));
  ...
  perform pg_temp.ok(27,'legacy writer service_role grant removed',not has_function_privilege('service_role','public.update_profile_verification(uuid,text,text)','EXECUTE'));
  perform pg_temp.ok(28,'reader source has no legacy verification fallback',strpos((select prosrc from pg_proc where oid='public.get_reservation_customer_profiles_v1(uuid[])'::regprocedure),'profile.verification_status')=0 and strpos((select prosrc from pg_proc where oid='public.admin_list_users_v1(integer,integer,text,text,text,text)'::regprocedure),'profile.verification_status')=0);
