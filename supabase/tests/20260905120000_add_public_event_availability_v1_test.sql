...
begin
  ...
  into v_result
  from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability;
  execute 'reset role';
...
begin
  ...
  insert into public.events (
    tenant_id, id, title, description, event_date, start_time, end_time, location,
    price, max_participants, is_active
  ) values
    (v_tenant, v_event_available, '[TEST][AVAILABILITY] A', 'Opis', current_date + 100, time '09:00', time '10:00', '[TEST]', 100, 10, true),
    (v_tenant, v_event_reserve, '[TEST][AVAILABILITY] B', 'Opis', current_date + 101, time '09:00', time '10:00', '[TEST]', 100, 10, true),
    (v_tenant, v_event_full, '[TEST][AVAILABILITY] C', 'Opis', current_date + 102, time '09:00', time '10:00', '[TEST]', 100, 2, true),
    (v_tenant, v_event_cancel, '[TEST][AVAILABILITY] D', 'Opis', current_date + 103, time '09:00', time '10:00', '[TEST]', 100, 3, true),
    (v_tenant, v_event_promote, '[TEST][AVAILABILITY] E', 'Opis', current_date + 104, time '09:00', time '10:00', '[TEST]', 100, 2, true),
    (v_tenant, v_event_writer, '[TEST][AVAILABILITY] F', 'Opis', current_date + 105, time '09:00', time '10:00', '[TEST]', 100, 1, true),
    (v_tenant, v_event_inactive, '[TEST][AVAILABILITY] Z', 'Opis', current_date + 106, time '09:00', time '10:00', '[TEST]', 100, 10, false);
  ...
  insert into public.event_registrations (
    ...
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_available, v_user_1, '[TEST] 1', 'availability-1-' || v_run || '@example.invalid', '000', 'registered', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_available, v_user_2, '[TEST] 2', 'availability-2-' || v_run || '@example.invalid', '000', 'approved', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_available, v_user_3, '[TEST] 3', 'availability-3-' || v_run || '@example.invalid', '000', ' Registered ', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_reserve, v_user_1, '[TEST] 1', 'availability-1-' || v_run || '@example.invalid', '000', 'registered', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_reserve, v_user_2, '[TEST] 2', 'availability-2-' || v_run || '@example.invalid', '000', 'reserve', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_reserve, v_user_3, '[TEST] 3', 'availability-3-' || v_run || '@example.invalid', '000', ' Reserve ', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_reserve, v_user_4, '[TEST] 4', 'availability-4-' || v_run || '@example.invalid', '000', 'cancelled', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_reserve, v_user_5, '[TEST] 5', 'availability-5-' || v_run || '@example.invalid', '000', 'participant', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_full, v_user_1, '[TEST] 1', 'availability-1-' || v_run || '@example.invalid', '000', 'registered', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_full, v_user_2, '[TEST] 2', 'availability-2-' || v_run || '@example.invalid', '000', 'approved', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_cancel, v_user_1, '[TEST] 1', 'availability-1-' || v_run || '@example.invalid', '000', 'registered', null, null),
    (v_tenant, v_cancel_registration, v_event_cancel, v_user_2, '[TEST] 2', 'availability-2-' || v_run || '@example.invalid', '000', 'approved', null, null),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_promote, v_user_1, '[TEST] 1', 'availability-1-' || v_run || '@example.invalid', '000', 'registered', null, null),
    (v_tenant, v_promote_registration, v_event_promote, v_user_2, '[TEST] 2', 'availability-2-' || v_run || '@example.invalid', '000', 'reserve', v_promotion_token, pg_catalog.now() + interval '1 day'),
    (v_tenant, pg_catalog.gen_random_uuid(), v_event_writer, v_user_1, '[TEST] 1', 'availability-1-' || v_run || '@example.invalid', '000', 'registered', null, null);
  ...
  perform pg_temp.record_result(1, 'Exact RPC signature exists without overloads',
    ...
    and (select pg_catalog.count(*) = 1 from pg_catalog.pg_proc as procedure join pg_catalog.pg_namespace as namespace on namespace.oid = procedure.pronamespace where namespace.nspname = 'public' and procedure.proname = 'get_public_event_availability_v2'),
    'Exactly one selected-tenant availability RPC must exist.');

  perform pg_temp.record_result(2, 'RPC is stable SECURITY DEFINER owned by postgres',
    exists (select 1 from pg_catalog.pg_proc as procedure join pg_catalog.pg_roles as owner_role on owner_role.oid = procedure.proowner where procedure.oid = 'public.get_public_event_availability_v2(uuid)'::regprocedure and procedure.prosecdef and procedure.provolatile = 's' and procedure.proconfig = array['search_path=pg_catalog, public, pg_temp']::text[] and owner_role.rolname = 'postgres'),
  ...
  perform pg_temp.record_result(3, 'RPC ACL is anon and authenticated only',
    pg_catalog.has_function_privilege('anon', 'public.get_public_event_availability_v2(uuid)', 'EXECUTE')
    and pg_catalog.has_function_privilege('authenticated', 'public.get_public_event_availability_v2(uuid)', 'EXECUTE')
    and not pg_catalog.has_function_privilege('service_role', 'public.get_public_event_availability_v2(uuid)', 'EXECUTE')
    and not exists (select 1 from pg_catalog.pg_proc as procedure cross join lateral pg_catalog.aclexplode(coalesce(procedure.proacl, pg_catalog.acldefault('f', procedure.proowner))) as acl where procedure.oid = 'public.get_public_event_availability_v2(uuid)'::regprocedure and acl.grantee = 0 and acl.privilege_type = 'EXECUTE'),
  ...
  select pg_catalog.to_jsonb(availability) into v_result from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_available;
  ...
  select pg_catalog.to_jsonb(availability) into v_result from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_reserve;
  ...
  select pg_catalog.to_jsonb(availability) into v_result from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_full;
  ...
  perform pg_temp.record_result(11, 'Public response has an exact PII-free shape',
    not exists (select 1 from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability cross join lateral pg_catalog.jsonb_object_keys(pg_catalog.to_jsonb(availability)) as key_name where key_name not in ('event_id','title','description','event_date','start_time','end_time','location','price','max_participants','registered_count','reserve_count','available_spots','sold_out')),
  ...
  select pg_catalog.to_jsonb(availability) into v_before from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_cancel;
  ...
  execute 'reset role';
  select pg_catalog.to_jsonb(availability) into v_after from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_cancel;
  ...
  select pg_catalog.to_jsonb(availability) into v_before from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_promote;
  perform pg_temp.set_client('authenticated', v_user_2);
  select public.confirm_event_reserve_promotion(v_promotion_token) into v_result;
  execute 'reset role';
  select pg_catalog.to_jsonb(availability) into v_after from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_promote;
  ...
  select pg_catalog.to_jsonb(availability) into v_after from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) as availability where availability.event_id = v_event_writer;
  ...
  perform pg_temp.record_result(21, 'Multiple-user aggregate is deterministic and de-duplicated',
    (select pg_catalog.count(*) = 1 from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) where event_id = v_event_available)
    and (select registered_count = 3 from public.get_public_event_availability_v2('c5c00000-0000-4000-8000-000000000001'::uuid) where event_id = v_event_available),
