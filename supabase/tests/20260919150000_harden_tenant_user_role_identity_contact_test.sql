...
begin
  ...
  perform pg_temp.ok(4,'list body frozen',md5(regexp_replace(pg_get_functiondef('public.admin_list_users_v1(integer,integer,text,text,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='2a95b1f3ba9c404adfa84f7eb9b8d425','list drifted');
  ...
  perform pg_temp.ok(41,'account-wide functions frozen',md5(regexp_replace(pg_get_functiondef('public.export_my_data_v1()'::regprocedure),E'\r\n?',E'\n','g'))='ffa6b35c5502a347e463110401032061' and md5(regexp_replace(pg_get_functiondef('public.anonymize_my_account_v1()'::regprocedure),E'\r\n?',E'\n','g'))='7e4d950e75e6e5782b139f11269d03a0','account lifecycle drift');
  perform pg_temp.ok(42,'verification function frozen',md5(regexp_replace(pg_get_functiondef('public.update_profile_verification(uuid,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='a0522b6beb94bde3bdff22799afc1368','verification drift');
  perform pg_temp.ok(43,'profile privilege trigger frozen',md5(regexp_replace(pg_get_functiondef('public.prevent_non_admin_profile_privilege_changes()'::regprocedure),E'\r\n?',E'\n','g'))='d28cb697d8355a5e8005296a03ad63ea','profile trigger drift');
  perform pg_temp.ok(44,'SECURITY DEFINER count remains 67',(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=67,'definer count differs');
