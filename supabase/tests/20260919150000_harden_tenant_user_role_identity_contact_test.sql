...
begin
  ...
  perform pg_temp.ok(4,'list body matches approved 4B-2B cutover',md5(regexp_replace(pg_get_functiondef('public.admin_list_users_v2(uuid,integer,integer,text,text,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='7333cc06535c24927ceaf4bd4643b690','list drifted');
  ...
  perform pg_temp.ok(42,'verification function matches approved 4B-2C closure',md5(regexp_replace(pg_get_functiondef('public.update_tenant_profile_verification_v2(uuid,uuid,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='30f1028aa801afc1abd0df080a4fd17f','verification drift');
  ...
  perform pg_temp.ok(44,'SECURITY DEFINER count is 75 after PRODUCT-10A public directory',(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=75,'definer count differs');
