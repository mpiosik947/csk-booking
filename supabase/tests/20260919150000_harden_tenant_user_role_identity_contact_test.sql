...
begin
  ...
  perform pg_temp.ok(42,'verification function matches approved 4B-2B cutover',md5(regexp_replace(pg_get_functiondef('public.update_profile_verification(uuid,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='8df439041f082c25e18a632f952623cb','verification drift');
  ...
  perform pg_temp.ok(44,'SECURITY DEFINER count is 69 after 9D-4B-2B',(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=69,'definer count differs');
