...
create function pg_temp.role_call(p_uid uuid,p_tenant uuid,p_target uuid,p_role text) returns jsonb language plpgsql as $f$
declare r jsonb; begin perform pg_temp.set_client(p_uid); select public.admin_set_user_role_v2(p_tenant,p_target,p_role) into r; reset role; return r;
exception when others then reset role; raise; end;$f$;
create function pg_temp.identity_call(p_uid uuid,p_tenant uuid,p_target uuid,p_first text,p_last text) returns jsonb language plpgsql as $f$
declare r jsonb; begin perform pg_temp.set_client(p_uid); select public.update_tenant_profile_identity_v2(p_tenant,p_target,p_first,p_last) into r; reset role; return r;
exception when others then reset role; raise; end;$f$;
create function pg_temp.contact_call(p_uid uuid,p_tenant uuid,p_target uuid,p_phone text) returns jsonb language plpgsql as $f$
declare r jsonb; begin perform pg_temp.set_client(p_uid); select public.update_tenant_profile_contact_details_v2(p_tenant,p_target,p_phone,'00-001','Warszawa','Testowa','1',null) into r; reset role; return r;
exception when others then reset role; raise; end;$f$;
create function pg_temp.identity_denied(p_uid uuid,p_tenant uuid,p_target uuid) returns boolean language plpgsql as $f$
begin perform pg_temp.identity_call(p_uid,p_tenant,p_target,'Denied','Target'); return false; exception when insufficient_privilege then return true; end;$f$;
create function pg_temp.contact_denied(p_uid uuid,p_tenant uuid,p_target uuid) returns boolean language plpgsql as $f$
begin perform pg_temp.contact_call(p_uid,p_tenant,p_target,'999'); return false; exception when insufficient_privilege then return true; end;$f$;
...
do $tests$
declare
  tenant_a uuid:='c5c00000-0000-4000-8000-000000000001'::uuid; tenant_b uuid:=pg_catalog.gen_random_uuid();
...
begin
  ...
  perform pg_temp.ok(1,'pre-change scope has exact signatures',(select count(*)=5 from pg_proc where oid in('public.admin_set_user_role_v2(uuid,uuid,text)'::regprocedure,'public.update_tenant_profile_identity_v2(uuid,uuid,text,text)'::regprocedure,'public.update_tenant_profile_contact_details_v2(uuid,uuid,text,text,text,text,text,text)'::regprocedure,'public.set_audit_log_tenant_id()'::regprocedure,'public.admin_list_users_v2(uuid,integer,integer,text,text,text,text)'::regprocedure)),'signature drift');
  perform pg_temp.ok(2,'three writers are authenticated-only DEFINER SP1',not exists(select 1 from pg_proc p join pg_roles o on o.oid=p.proowner where p.oid in('public.admin_set_user_role_v2(uuid,uuid,text)'::regprocedure,'public.update_tenant_profile_identity_v2(uuid,uuid,text,text)'::regprocedure,'public.update_tenant_profile_contact_details_v2(uuid,uuid,text,text,text,text,text,text)'::regprocedure) and not(p.prosecdef and o.rolname='postgres' and p.proconfig=array['search_path=pg_catalog, public, pg_temp']::text[] and has_function_privilege('authenticated',p.oid,'EXECUTE') and not has_function_privilege('public',p.oid,'EXECUTE') and not has_function_privilege('anon',p.oid,'EXECUTE') and not has_function_privilege('service_role',p.oid,'EXECUTE'))),'metadata/ACL differs');
  ...
  perform pg_temp.ok(4,'list body includes approved PRODUCT-10D staff entitlement gate',md5(regexp_replace(pg_get_functiondef('public.admin_list_users_v2(uuid,integer,integer,text,text,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='aa83a0f9cb00b3b00dc0ff1294b1ffa2','list drifted');

  r:=pg_temp.role_call(admin_a,tenant_a,user_a,'pracownik');
  ...
  r:=pg_temp.role_call(admin_a,tenant_a,user_a,'instruktor');
  ...
  r:=pg_temp.role_call(admin_a,tenant_a,user_a,'user');
  ...
  r:=pg_temp.role_call(admin_a,tenant_a,user_a,'admin');
  ...
  r:=pg_temp.role_call(admin_a,tenant_a,user_a,'user');
  perform pg_temp.ok(10,'cross-tenant role mutation denied',pg_temp.role_call(admin_a,tenant_a,user_b,'admin')->>'code'='not_allowed' and (select role='user' from public.tenant_memberships where tenant_id=tenant_b and user_id=user_b),'cross-tenant role changed');
  perform pg_temp.ok(11,'unrelated role mutation denied',pg_temp.role_call(admin_a,tenant_a,unrelated,'admin')->>'code'='not_allowed','unrelated role allowed');
  perform pg_temp.ok(12,'global role without membership denied',pg_temp.role_call(global_admin,tenant_a,user_a,'user')->>'code'='not_allowed','global role bypass');
  perform pg_temp.ok(13,'pending membership denied',pg_temp.role_call(pending_admin,tenant_a,user_a,'user')->>'code'='not_allowed','pending allowed');
  perform pg_temp.ok(14,'suspended membership denied',pg_temp.role_call(suspended_admin,tenant_a,user_a,'user')->>'code'='not_allowed','suspended allowed');

  r:=pg_temp.identity_call(admin_a,tenant_a,user_a,'Tenant','Identity');
  ...
  perform pg_temp.ok(16,'identity B-only denied',pg_temp.identity_denied(admin_a,tenant_a,user_b),'B identity allowed');
  perform pg_temp.ok(17,'identity unrelated denied',pg_temp.identity_denied(admin_a,tenant_a,unrelated),'unrelated identity allowed');
  perform pg_temp.ok(18,'identity global-role bypass removed',pg_temp.identity_denied(global_admin,tenant_a,user_a),'global identity bypass');
  perform pg_temp.ok(19,'identity employee denied',pg_temp.identity_denied(employee_a,tenant_a,user_a),'employee identity allowed');
  perform pg_temp.ok(20,'identity instructor denied',pg_temp.identity_denied(instructor_a,tenant_a,user_a),'instructor identity allowed');

  r:=pg_temp.contact_call(admin_a,tenant_a,user_a,'111');
  ...
  r:=pg_temp.contact_call(employee_a,tenant_a,user_a,'222');
  ...
  perform pg_temp.ok(23,'employee self contact denied',pg_temp.contact_denied(employee_a,tenant_a,employee_a),'employee self allowed');
  perform pg_temp.ok(24,'employee admin target denied',pg_temp.contact_denied(employee_a,tenant_a,admin_a),'employee admin target allowed');
  perform pg_temp.ok(25,'employee instructor target denied',pg_temp.contact_denied(employee_a,tenant_a,instructor_a),'employee instructor target allowed');
  perform pg_temp.ok(26,'contact B-only denied',pg_temp.contact_denied(admin_a,tenant_a,user_b),'B contact allowed');
  perform pg_temp.ok(27,'contact unrelated denied',pg_temp.contact_denied(admin_a,tenant_a,unrelated),'unrelated contact allowed');
  perform pg_temp.ok(28,'contact global-role bypass removed',pg_temp.contact_denied(global_admin,tenant_a,user_a),'global contact bypass');

  select count(*) into before_audit from public.audit_logs where target_id=user_a and action like 'tenant_user_%';
  r:=pg_temp.identity_call(admin_a,tenant_a,user_a,'Tenant','Identity Two');
  r:=pg_temp.contact_call(admin_a,tenant_a,user_a,'333');
  r:=pg_temp.role_call(admin_a,tenant_a,user_a,'pracownik');
  ...
  r:=pg_temp.role_call(admin_b,tenant_b,user_b,'admin');
  r:=pg_temp.role_call(admin_b,tenant_b,user_b,'user');
  perform pg_temp.ok(32,'one of multiple active admins may be demoted',r->>'code'='updated','multi-admin demotion denied');
  r:=pg_temp.role_call(admin_b,tenant_b,admin_b,'user');
  ...
  r:=pg_temp.role_call(admin_b,tenant_b,user_b,'instruktor');
  ...
  perform pg_temp.ok(37,'role function has tenant serialization',(select prosrc like '%pg_advisory_xact_lock%' and prosrc like '%status=''active''%and membership.role=''admin''%' from pg_proc where oid='public.admin_set_user_role_v2(uuid,uuid,text)'::regprocedure),'serialization predicate missing');
  perform pg_temp.ok(38,'operational relationship sources are explicit',not exists(select 1 from pg_proc where oid in('public.update_tenant_profile_identity_v2(uuid,uuid,text,text)'::regprocedure,'public.update_tenant_profile_contact_details_v2(uuid,uuid,text,text,text,text,text,text)'::regprocedure) and not(prosrc like '%tenant_memberships%' and prosrc like '%reservations%' and prosrc like '%event_registrations%')),'relationship source missing');
  perform pg_temp.ok(39,'global profile role removed from writer authority',not exists(select 1 from pg_proc where oid in('public.admin_set_user_role_v2(uuid,uuid,text)'::regprocedure,'public.update_tenant_profile_identity_v2(uuid,uuid,text,text)'::regprocedure,'public.update_tenant_profile_contact_details_v2(uuid,uuid,text,text,text,text,text,text)'::regprocedure) and prosrc~'profile[.]role'),'global role authority remains');
  ...
  perform pg_temp.ok(41,'account-wide functions frozen',md5(regexp_replace(pg_get_functiondef('public.export_my_data_v1()'::regprocedure),E'\r\n?',E'\n','g'))='d159b7d0a14f7ffc9d6c3e5088d18dc5' and md5(regexp_replace(pg_get_functiondef('public.anonymize_my_account_v1()'::regprocedure),E'\r\n?',E'\n','g'))='c44c385685f00f36449c5ae80a9e81ce','account lifecycle drift');
  perform pg_temp.ok(42,'verification function includes approved PRODUCT-10D staff entitlement gate',md5(regexp_replace(pg_get_functiondef('public.update_tenant_profile_verification_v2(uuid,uuid,text,text)'::regprocedure),E'\r\n?',E'\n','g'))='ce01bdd4ba810ff578d063ca5a13db4b','verification drift');
  ...
  perform pg_temp.ok(44,'SECURITY DEFINER count is 100 after PRODUCT-10C public landing',(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef)=  104,'definer count differs');
  ...
  perform pg_temp.ok(47,'list tenant note contract remains active',(select prosrc like '%tenant_user_admin_notes%' and prosrc not like '%profile.admin_note%' from pg_proc where oid='public.admin_list_users_v2(uuid,integer,integer,text,text,text,text)'::regprocedure),'note/list regression');
end;$tests$;
