...
begin
  ...
  perform pg_temp.ok(32,'SECURITY DEFINER inventory reflects the approved 2C-1 invoker conversion',(select count(*)=72 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'definer inventory drifted');
end;$tests$;
