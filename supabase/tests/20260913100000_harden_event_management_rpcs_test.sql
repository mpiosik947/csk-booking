...
begin
  ...
  perform pg_temp.ok(1,'four tenant-scoped event contracts exist',(select count(*)=4 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('admin_create_event_v3','admin_update_event_v3','admin_set_event_active_v3','admin_list_events_v2')),'wrapper inventory differs');
  perform pg_temp.ok(2,'writers are postgres-owned SP1 definers and list is a closed invoker',(select count(*)=4 and bool_and(r.rolname='postgres') and bool_and(p.proconfig=array['search_path=pg_catalog, public, pg_temp']::text[]) and count(*) filter(where p.prosecdef)=3 and count(*) filter(where not p.prosecdef)=1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace join pg_roles r on r.oid=p.proowner where n.nspname='public' and p.proname in('admin_create_event_v3','admin_update_event_v3','admin_set_event_active_v3','admin_list_events_v2')),'wrapper metadata differs');
  perform pg_temp.ok(3,'current create core is invoker-only and inaccessible',(select count(*)=1 and bool_and(not p.prosecdef) and bool_and(not has_function_privilege('public',p.oid,'EXECUTE')) and bool_and(not has_function_privilege('anon',p.oid,'EXECUTE')) and bool_and(not has_function_privilege('authenticated',p.oid,'EXECUTE')) and bool_and(not has_function_privilege('service_role',p.oid,'EXECUTE')) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='admin_create_event_v3__saas9ec2a_core'),'core isolation differs');
  perform pg_temp.ok(4,'active client ACL is authenticated-only',(select count(*)=4 and bool_and(not has_function_privilege('public',p.oid,'EXECUTE')) and bool_and(not has_function_privilege('anon',p.oid,'EXECUTE')) and bool_and(has_function_privilege('authenticated',p.oid,'EXECUTE')) and bool_and(not has_function_privilege('service_role',p.oid,'EXECUTE')) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('admin_create_event_v3','admin_update_event_v3','admin_set_event_active_v3','admin_list_events_v2')),'active grants differ');
  perform pg_temp.ok(5,'current event writer fingerprints are approved',(
    select count(*)=3
      and bool_and(md5(replace(replace(pg_get_functiondef(p.oid),E'\r\n',E'\n'),E'\r',E'\n'))=expected.fingerprint)
      and bool_and(p.prosecdef)
      and bool_and(r.rolname='postgres')
      and bool_and(p.proconfig=array['search_path=pg_catalog, public, pg_temp']::text[])
  ...
  ),'current body/signature/owner/search_path or ACL changed');
  perform pg_temp.ok(6,'create core explicitly writes event and relation tenant',strpos(pg_get_functiondef('public.admin_create_event_v3__saas9ec2a_core(uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])'::regprocedure),'tenant_id')>0 and strpos(pg_get_functiondef('public.admin_create_event_v3__saas9ec2a_core(uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])'::regprocedure),'p_tenant_id')>0,'create tenant ownership absent');
  perform pg_temp.ok(7,'update writer binds event and requested lanes to explicit tenant',strpos(pg_get_functiondef('public.admin_update_event_v3(uuid,uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])'::regprocedure),'p_tenant_id')>0 and strpos(pg_get_functiondef('public.admin_update_event_v3(uuid,uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])'::regprocedure),'lane.tenant_id is distinct from p_tenant_id')>0,'update tenant binding absent');
  perform pg_temp.ok(8,'admin list and create contract are explicitly tenant-scoped',strpos(pg_get_functiondef('public.admin_list_events_v2(uuid,text,text,text,integer,integer)'::regprocedure),'p_tenant_id')>0 and strpos(pg_get_functiondef('public.admin_create_event_v3(uuid,text,text,date,time without time zone,time without time zone,text,numeric,integer,uuid[])'::regprocedure),'p_tenant_id')>0,'explicit tenant scope absent');

  result:=pg_temp.as_actor_json('authenticated',admin_a,format('select public.admin_create_event_v3(%L,%L,%L,date ''2099-11-03'',time ''10:00'',time ''11:00'',%L,100,10,array[%L]::uuid[])',csk,marker||' Created A','A','Test',lane_a));
  ...
  result:=pg_temp.as_actor_json('authenticated',employee_a,format('select public.admin_create_event_v3(%L,%L,%L,date ''2099-11-04'',time ''10:00'',time ''11:00'',%L,100,10,array[]::uuid[])',csk,marker||' Employee A','A','Test'));
  perform pg_temp.ok(11,'employee preserves create permission',result->>'code'='created','employee create failed');
  perform pg_temp.ok(12,'instructor and ordinary user cannot create',(pg_temp.as_actor_json('authenticated',instructor_a,format('select public.admin_create_event_v3(%L,%L,%L,date ''2099-11-05'',time ''10:00'',time ''11:00'',%L,100,10,array[]::uuid[])',csk,marker||' Instructor','A','Test')))->>'code'='not_allowed' and (pg_temp.as_actor_json('authenticated',user_a,format('select public.admin_create_event_v3(%L,%L,%L,date ''2099-11-05'',time ''12:00'',time ''13:00'',%L,100,10,array[]::uuid[])',csk,marker||' User','A','Test')))->>'code'='not_allowed','management permission widened');
  select count(*) into before_count from public.events;
  result:=pg_temp.as_actor_json('authenticated',admin_a,format('select public.admin_create_event_v3(%L,%L,%L,date ''2099-11-06'',time ''10:00'',time ''11:00'',%L,100,10,array[%L]::uuid[])',csk,marker||' Mixed','A','Test',lane_b));
  ...
  result:=pg_temp.as_actor_json('authenticated',employee_a,format('select public.admin_update_event_v3(%L,%L,%L,%L,date ''2099-11-01'',time ''11:00'',time ''12:00'',%L,120,12,array[%L]::uuid[])',csk,event_a,marker||' Event A updated','A2','Test',lane_a));
  ...
  result:=pg_temp.as_actor_json('authenticated',admin_a,format('select public.admin_update_event_v3(%L,%L,%L,%L,date ''2099-11-02'',time ''11:00'',time ''12:00'',%L,120,12,array[%L]::uuid[])',csk,event_b,marker||' Event B blocked','B2','Test',lane_b));
  ...
  result:=pg_temp.as_actor_json('authenticated',admin_a,format('select public.admin_update_event_v3(%L,%L,%L,%L,date ''2099-11-01'',time ''12:00'',time ''13:00'',%L,120,12,array[%L]::uuid[])',csk,event_a,marker||' Bad lanes','A3','Test',lane_b));
  ...
  result:=pg_temp.as_actor_json('authenticated',admin_a,format('select public.admin_set_event_active_v3(%L,%L,false)',csk,event_a));
  ...
  result:=pg_temp.as_actor_json('authenticated',employee_a,format('select public.admin_set_event_active_v3(%L,%L,true)',csk,event_a));
  ...
  perform pg_temp.ok(20,'instructor cannot activate event',(pg_temp.as_actor_json('authenticated',instructor_a,format('select public.admin_set_event_active_v3(%L,%L,false)',csk,event_a)))->>'code'='not_allowed','instructor management widened');
  perform pg_temp.ok(21,'global admin without membership is denied',(pg_temp.as_actor_json('authenticated',no_member_admin,format('select public.admin_set_event_active_v3(%L,%L,false)',csk,event_a)))->>'code'='not_allowed','global profile role bypass remains');
  perform pg_temp.ok(22,'pending and suspended admins are denied',(pg_temp.as_actor_json('authenticated',pending_admin,format('select public.admin_set_event_active_v3(%L,%L,false)',csk,event_a)))->>'code'='not_allowed' and (pg_temp.as_actor_json('authenticated',suspended_admin,format('select public.admin_set_event_active_v3(%L,%L,false)',csk,event_a)))->>'code'='not_allowed','inactive membership authorized');

  result:=pg_temp.as_actor_json('authenticated',admin_a,format('select public.admin_list_events_v2(%L,%L,''all'',''nearest'',1,50)',csk,marker));
  ...
  perform pg_temp.ok(25,'instructor retains read-only admin-list scope',(pg_temp.as_actor_json('authenticated',instructor_a,format('select public.admin_list_events_v2(%L,%L,''all'',''nearest'',1,50)',csk,marker)))->>'code'='ok','instructor read scope regressed');
  perform pg_temp.ok(26,'ordinary and no-membership users cannot list',(pg_temp.as_actor_json('authenticated',user_a,format('select public.admin_list_events_v2(%L,null,''all'',''nearest'',1,20)',csk)))->>'code'='not_allowed' and (pg_temp.as_actor_json('authenticated',no_member_admin,format('select public.admin_list_events_v2(%L,null,''all'',''nearest'',1,20)',csk)))->>'code'='not_allowed','admin list role bypass remains');
  ...
  perform pg_temp.ok(32,'SECURITY DEFINER inventory reflects PRODUCT-10B public readers',(select count(*)=  104 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'definer inventory drifted');
end;$tests$;
