...
begin
  ...
  perform pg_temp.ok(32,'SECURITY DEFINER inventory reflects approved hardening through 9D-3C',(select count(*)=67 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef),'definer inventory drifted');
end;$tests$;
