...
if (-not $ConfirmIsolatedDatabase) {
  throw 'Concurrency harness is blocked. Use only an isolated schema-only database.'
}

$migrationPath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\migrations\20260809210000_add_atomic_lane_booking_configuration_rpc.sql'))
$marker = '[TEST][6B-4D1][CONCURRENCY]'
$runId = [Guid]::NewGuid().ToString('N').Substring(0, 10)
$workDir = Join-Path $env:TEMP "csk-6b4d1-concurrency-$runId"
New-Item -ItemType Directory -Path $workDir | Out-Null

$ids = @{
  Admin = '6b4d1c00-0000-4000-8000-000000000001'
  User = '6b4d1c00-0000-4000-8000-000000000002'
  RootA = '6b4d1c00-0000-4000-8000-000000000101'
  A1 = '6b4d1c00-0000-4000-8000-000000000102'
  A2 = '6b4d1c00-0000-4000-8000-000000000103'
  RootB = '6b4d1c00-0000-4000-8000-000000000201'
  B1 = '6b4d1c00-0000-4000-8000-000000000202'
  Standalone = '6b4d1c00-0000-4000-8000-000000000301'
}

$migrationIncludePath = $migrationPath
if ($DockerContainerName) {
  $migrationIncludePath = "/tmp/csk-6b4d1-migration-$runId.sql"
...
function Get-PsqlLaunch {
  ...
  if ($DockerContainerName) {
    $containerPath = "/tmp/csk-6b4d1-$runId-$([System.IO.Path]::GetFileName($Path))"
...
function Start-PsqlFile {
  ...
  $process = Start-Process -FilePath $launch.FilePath `
...
function Get-PricingJson {
  param([int]$Price)
  return "jsonb_build_array(jsonb_build_object('day_group','mon_thu','min_shooters',1,'max_shooters',5,'label','$marker','hourly_price',$Price),jsonb_build_object('day_group','fri_sun','min_shooters',1,'max_shooters',5,'label','$marker','hourly_price',$($Price + 20)))"
}

function Get-ConfigSql {
  param([string]$LaneId,[int]$Price,[string]$Durations = 'array[60]')
  $pricing = Get-PricingJson $Price
  return "public.admin_set_lane_booking_configuration('$LaneId',true,true,true,5,true,5,$Durations,$pricing)"
}

function Get-PositionConfigSql {
  param([string]$LaneId,[int]$Price)
  $pricing = Get-PricingJson $Price
  return "public.admin_set_lane_booking_configuration('$LaneId',true,false,false,5,true,5,array[60],$pricing)"
}

$setup = New-SqlFile 'setup' @"
do `$preflight`$
begin
  if pg_catalog.to_regprocedure('public.admin_set_lane_booking_configuration(uuid,boolean,boolean,boolean,integer,boolean,integer,integer[],jsonb)') is not null
     or exists(select 1 from public.shooting_lanes where name like '$marker%')
     or exists(select 1 from auth.users where email like 'test-6b4d1-concurrency-%@example.invalid') then
    raise exception 'Concurrency preflight found existing RPC or fixtures.';
  end if;
end;
`$preflight`$;
...
insert into auth.users(id,instance_id,aud,role,email,encrypted_password,raw_app_meta_data,raw_user_meta_data,created_at,updated_at) values
  ('$($ids.Admin)','00000000-0000-0000-0000-000000000000','authenticated','authenticated','test-6b4d1-concurrency-admin@example.invalid','','{}','{}',pg_catalog.transaction_timestamp(),pg_catalog.transaction_timestamp()),
  ('$($ids.User)','00000000-0000-0000-0000-000000000000','authenticated','authenticated','test-6b4d1-concurrency-user@example.invalid','','{}','{}',pg_catalog.transaction_timestamp(),pg_catalog.transaction_timestamp());
insert into public.profiles(user_id,role,first_name,last_name,full_name,email,phone,verification_status)
values
  ('$($ids.Admin)','admin','[TEST]','6B-4D1-CONCURRENCY','$marker','test-6b4d1-concurrency-profile@example.invalid','000000000','verified'),
  ('$($ids.User)','user','[TEST]','6B-4D1-CONCURRENCY','$marker','test-6b4d1-concurrency-profile@example.invalid','000000000','verified')
on conflict (user_id) do update set role=excluded.role,first_name=excluded.first_name,last_name=excluded.last_name,full_name=excluded.full_name,email=excluded.email,phone=excluded.phone,verification_status=excluded.verification_status;
insert into public.shooting_lanes(id,name,type,description,price_per_hour,is_active,max_shooters,booking_step_minutes,display_order,currency_code,resource_kind,parent_lane_id,whole_lane_bookable,positions_bookable) values
  ...
  ('$($ids.Standalone)','$marker[STANDALONE]','[TEST]','[TEST]',10,true,5,60,9990,'PLN','lane',null,true,false);
...
select id,60,10,true from public.shooting_lanes where name like '$marker%';
...
from public.shooting_lanes lane cross join (values('mon_thu'::text),('fri_sun'::text)) day_group(value)
where lane.name like '$marker%';
"@

$cleanup = New-SqlFile 'cleanup' @"
begin;
delete from public.audit_logs where target_id in (select id from public.reservations where reservation_note='$marker');
...
delete from auth.users where email like 'test-6b4d1-concurrency-%@example.invalid';
drop function if exists public.admin_set_lane_booking_configuration(uuid,boolean,boolean,boolean,integer,boolean,integer,integer[],jsonb);
commit;
select 'cleanup_passed=' || (
  pg_catalog.to_regprocedure('public.admin_set_lane_booking_configuration(uuid,boolean,boolean,boolean,integer,boolean,integer,integer[],jsonb)') is null
  ...
  and not exists(select 1 from auth.users where email like 'test-6b4d1-concurrency-%@example.invalid')
)::text;
"@
...
try {
  ...
  $reservation = {
    param($lane,$dateOffset,$request,$duration=60)
    "public.create_reservation_v2('$lane',current_date+$dateOffset,time '10:00',$duration,1,'$request','$marker')"
  }

  $results += Invoke-Scenario 1 'config parent first vs child reservation' $ids.Admin `
    (Get-ConfigSql $ids.RootA 110) 'updated' $ids.User `
    (&$reservation $ids.A1 8101 '6b4d1c00-0000-4000-8000-000000001001') 'created' ($HoldSeconds-2.5) ($HoldSeconds+8) `
    "select 'check_passed='||(exists(select 1 from public.reservations where reservation_note='$marker' and lane_id='$($ids.A1)' and reservation_date=current_date+8101))::text;"

  $results += Invoke-Scenario 2 'child reservation first vs parent config' $ids.User `
    (&$reservation $ids.A1 8102 '6b4d1c00-0000-4000-8000-000000001002') 'created' $ids.Admin `
    (Get-ConfigSql $ids.RootA 120) 'updated' ($HoldSeconds-2.5) ($HoldSeconds+8) `
    "select 'check_passed='||(exists(select 1 from public.reservations where reservation_note='$marker' and lane_id='$($ids.A1)' and reservation_date=current_date+8102))::text;"

  $results += Invoke-Scenario 3 'config child1 vs reservation child2' $ids.Admin `
    (Get-PositionConfigSql $ids.A1 130) 'updated' $ids.User `
    (&$reservation $ids.A2 8103 '6b4d1c00-0000-4000-8000-000000001003') 'created' 0 2.5 `
    "select 'check_passed='||(exists(select 1 from public.reservations where reservation_note='$marker' and lane_id='$($ids.A2)' and reservation_date=current_date+8103))::text;"

  $results += Invoke-Scenario 4 'config parent vs child lane block' $ids.Admin `
    (Get-ConfigSql $ids.RootA 140) 'updated' $ids.Admin `
    "public.admin_create_lane_block('$($ids.A1)',current_date+8104,time '10:00',time '11:00','$marker[S4]')" 'created' ($HoldSeconds-2.5) ($HoldSeconds+8) `
    "select 'check_passed='||(exists(select 1 from public.lane_blocks where reason='$marker[S4]'))::text;"

  $results += Invoke-Scenario 5 'config parent vs child Event V2' $ids.Admin `
    (Get-ConfigSql $ids.RootA 150) 'updated' $ids.Admin `
    "public.admin_create_event_v2('$marker[S5]','$marker',current_date+8105,time '10:00',time '11:00','$marker',10,10,array['$($ids.A1)']::uuid[])" 'created' ($HoldSeconds-2.5) ($HoldSeconds+8) `
    "select 'check_passed='||(exists(select 1 from public.events where title='$marker[S5]'))::text;"

  $results += Invoke-Scenario 6 'independent roots remain parallel' $ids.Admin `
    (Get-ConfigSql $ids.RootA 160) 'updated' $ids.Admin `
    (Get-ConfigSql $ids.RootB 210) 'updated' 0 2.5 `
    "select 'check_passed='||((select hourly_price=160 from public.lane_pricing_rules where lane_id='$($ids.RootA)' and day_group='mon_thu' and is_active) and (select hourly_price=210 from public.lane_pricing_rules where lane_id='$($ids.RootB)' and day_group='mon_thu' and is_active))::text;"

  $results += Invoke-Scenario 7 'two config writers serialize' $ids.Admin `
    (Get-ConfigSql $ids.RootB 220) 'updated' $ids.Admin `
    (Get-ConfigSql $ids.RootB 230) 'updated' ($HoldSeconds-2.5) ($HoldSeconds+8) `
    "select 'check_passed='||(select hourly_price=230 from public.lane_pricing_rules where lane_id='$($ids.RootB)' and day_group='mon_thu' and is_active)::text;"

  $results += Invoke-Scenario 8 'snapshot replacement vs reservation' $ids.Admin `
    (Get-ConfigSql $ids.Standalone 170 'array[120]') 'updated' $ids.User `
    (&$reservation $ids.Standalone 8108 '6b4d1c00-0000-4000-8000-000000001008' 120) 'created' ($HoldSeconds-2.5) ($HoldSeconds+8) `
    "select 'check_passed='||(select duration_minutes=120 and pricing_label_snapshot='$marker' and price_per_hour_snapshot=170 from public.reservations where reservation_note='$marker' and lane_id='$($ids.Standalone)' and reservation_date=current_date+8108)::text;"
}
finally {
  if ($setupSucceeded) {
...
if ($results.Count -ne 8 -or ($results | Where-Object { -not $_.passed })) {
  throw 'One or more 6B-4D1 concurrency scenarios failed.'
}

Write-Output '6B-4D1 concurrency: 8/8 PASSED'
...
Write-Output 'partial_configuration=false'
