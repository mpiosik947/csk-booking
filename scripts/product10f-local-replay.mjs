...
import { domainConcurrency } from './product10f-concurrency.mjs';
...
try {
  ...
  const chain = readdirSync(migrationDir).filter(n => /^\d{14}_.*\.sql$/.test(n) && n <= '20261011100000_zzz.sql').sort();
  if (!chain.at(-1)?.startsWith('20261011100000_')) throw new Error('CHAIN_HEAD_MISMATCH');
  ...
  sql(scratch, replay);
  console.log('ISOLATED_MIGRATION_HEAD=20261011100000; COMPLETE_CHAIN=' + chain.length);
  ...
  if (inventory.trim() !== "104\nt") throw new Error("ISOLATED_INVENTORY_FAILED: " + inventory);
  console.log("ISOLATED_SECURITY_DEFINER=104; TCM_OBJECTS=PRESENT");
  ...
  const focused = readFileSync(new URL("../supabase/tests/20261011100000_verified_tenant_domains_test.sql", import.meta.url), "utf8");
  const output = sql(scratch, focused);
  if (/not ok/m.test(output)) throw new Error("FOCUSED_SQL_FAILED");
  console.log(output);
  await domainConcurrency(scratch);
  let assertions = 0, files = 0;
  const failures = [];
  const testDir = new URL('../supabase/tests/', import.meta.url);
  for (const name of readdirSync(testDir).filter(n => n.endsWith('.sql') && (!/^\d/.test(n) || n <= '20261011100000_zzz.sql')).sort()) {
    try {
