...
function preTcmTest(source, name) {
  ...
  if (name.startsWith('20260903100000')) s = s.replace('v_writer_count=27', 'v_writer_count=26');
...
try {
  ...
  const chain = readdirSync(migrationDir).filter(n => /^\d{14}_.*\.sql$/.test(n) && n <= '20261009150000_zzz.sql').sort();
  if (!chain.at(-1)?.startsWith('20261009150000_')) throw new Error('CHAIN_HEAD_MISMATCH');
  ...
  sql(scratch, replay);
  console.log('ISOLATED_MIGRATION_HEAD=20261009150000; COMPLETE_CHAIN=' + chain.length);
  ...
  const focused = readFileSync(new URL("../supabase/tests/20261009150000_global_platform_bootstrap_audit_test.sql", import.meta.url), "utf8");
  ...
  for (const name of readdirSync(testDir).filter(n => n.endsWith('.sql') && (!/^\d/.test(n) || n <= '20261009150000_zzz.sql')).sort()) {
