...
select public.admin_set_user_role_v2('${tenant}'::uuid,'${target}'::uuid,'user');
commit;`;
...
update public.profiles set role='admin',first_name='[TEST]',last_name='Concurrency A',full_name='[TEST][SAAS-9D-4B-1B] Concurrency A' where user_id='${adminA}';
update public.profiles set role='admin',first_name='[TEST]',last_name='Concurrency B',full_name='[TEST][SAAS-9D-4B-1B] Concurrency B' where user_id='${adminB}';
update public.profiles set role='user',first_name='[TEST]',last_name='Concurrent Customer',full_name='[TEST][SAAS-9D-4B-1B] Concurrent Customer' where user_id='${customer}';
insert into public.tenant_memberships(tenant_id,user_id,role,status)
values
('${tenant}','${adminA}','admin','active'),
('${tenant}','${adminB}','admin','active'),
...
try {
  ...
  await Promise.all([
    profileCall(remainingAdmin, `public.update_tenant_profile_identity_v2('${tenant}'::uuid,'${customer}'::uuid,'Concurrent','Identity')`),
    profileCall(remainingAdmin, `public.update_tenant_profile_contact_details_v2('${tenant}'::uuid,'${customer}'::uuid,'555000111','00-001','Warszawa','Concurrent','1',null)`),
  ]);
